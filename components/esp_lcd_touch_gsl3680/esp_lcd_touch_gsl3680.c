...
/* Optional raw-coordinate logging callback (see header). Registered by the
 * application to stream every reported finger to persistent storage. */
static esp_lcd_touch_gsl3680_raw_cb_t s_raw_cb = NULL;

void esp_lcd_touch_gsl3680_set_raw_cb(esp_lcd_touch_gsl3680_raw_cb_t cb)
{
    s_raw_cb = cb;
}
...
{
    esp_err_t err;
    /* The GSL3680 0x80 register returns 44 bytes (1 count byte + up to
     * 10 points * 4 bytes + 3 trailing bytes). The original buffer was only
     * 24 bytes, which overflowed the stack and clobbered callee-saved
     * registers, causing a NULL data pointer in lvgl_port_touchpad_read. */
    uint8_t touch_data[44];
    uint16_t x_poit, y_poit, x2_poit, y2_poit;

    assert(tp != NULL);

    /* Diagnostic: detect read starvation during an ACTIVE touch. In EVENT
     * mode read_data() runs when the GSL3680 INT pin fires; while a finger is
     * down the INT should re-assert for every report, so a large gap between
     * consecutive reads that both saw a finger means the LVGL task was busy
     * (CPU saturation). Gaps between separate touches are normal and ignored. */
    const uint32_t t_start_ms = esp_log_timestamp();
    static uint32_t s_last_poll_ms = 0;
    static bool s_last_had_finger = false;
    if (s_last_had_finger && s_last_poll_ms != 0) {
        const uint32_t gap_ms = t_start_ms - s_last_poll_ms;
        if (gap_ms > 50) {
            ESP_LOGW(TAG, "touch gap during active touch %lu ms (LVGL task starved?)",
...
// #endif
    ...
    if (Finger_num > MAX_FINGER_NUM) {
        /* Clamp to the driver's exposed point capacity so a noisy count byte
         * cannot overflow XY_Coordinate[]. */
    ...
    for(int j=0;j<Finger_num;j++)
    {
        ...
        /* Y is a 12-bit sense coordinate. The high nibble of the Y-high byte
         * carries point flags (key/fill/able/predict, bits 12..15), exactly
         * like the X-high byte carries the finger ID in its high nibble. The
         * vendor SDK only masks X (0x0f) but not Y, so a flag bit set by the
         * IC (e.g. "able" 0x40 in this panel's dead band) leaks into the
         * coordinate and makes gsl_alg_id_main() reject an otherwise valid
         * point as out-of-range/generated. Mask Y to 12 bits. */
        y_poit = touch_data[(j+1)*4+1] & 0x0f;
        y2_poit = touch_data[(j+1)*4+0];
        cinfo.y[j] = y_poit << 8 | y2_poit;
        cinfo.id[j] = ((touch_data[(j+1)*4+3] & 0xf0) >> 4);
    }

    /* Diagnostic: true raw I2C values BEFORE the ID algorithm, on each NEW
     * press. Logs the pre-algorithm coordinate so we can compare it against
     * the post-algorithm value (RAW press #N below) to localize the dead
     * band to the touch IC firmware vs the host-side gsl_alg_id_main(). */
    static uint32_t s_raw_pre_count = 0;
    static bool s_pre_had_finger = false;
    if (Finger_num > 0 && !s_pre_had_finger && s_raw_pre_count < 60) {
        s_raw_pre_count++;
        ESP_LOGI(TAG, "RAWpre press #%lu x=%u y=%u",
                 (unsigned long)s_raw_pre_count,
                 (unsigned)cinfo.x[0], (unsigned)cinfo.y[0]);
    }
    s_pre_had_finger = (Finger_num > 0);

    /* Capture pre-algorithm values so a finger dropped by the ID algorithm
     * can be diagnosed (dead band = IC drops vs host algorithm drops). */
    const uint16_t pre_alg_x0 = cinfo.x[0];
    const uint16_t pre_alg_y0 = cinfo.y[0];
    const uint8_t pre_alg_finger = Finger_num;

    gsl_alg_id_main(&cinfo);
    static uint32_t s_drop_count = 0;
    if (pre_alg_finger > 0 && cinfo.finger_num == 0 && s_drop_count < 60) {
        s_drop_count++;
        ESP_LOGW(TAG, "DROP alg: raw(f=%u x=%u y=%u) -> no point",
                 (unsigned)pre_alg_finger, (unsigned)pre_alg_x0, (unsigned)pre_alg_y0);
    }
    ...
    /* Stream every reported finger to the raw logger. Fires even when the
     * algorithm dropped all points (out_f == 0), which is exactly what a
     * dead-band tap looks like: raw_f > 0 but out_f == 0. */
    if (s_raw_cb != NULL && (pre_alg_finger > 0 || Finger_num > 0)) {
        const uint16_t out_x = (Finger_num > 0) ? (uint16_t)cinfo.x[0] : 0;
        const uint16_t out_y = (Finger_num > 0) ? (uint16_t)cinfo.y[0] : 0;
        s_raw_cb(t_start_ms, pre_alg_x0, pre_alg_y0, pre_alg_finger,
                 out_x, out_y, (uint8_t)Finger_num);
    }
    ...
    /* Diagnostic: log raw panel-native coordinates on each NEW press (the
     * transition from no-finger to finger). s_last_had_finger still holds the
     * previous read's value at this point. */
    static uint32_t s_raw_log_count = 0;
    if (Finger_num > 0 && !s_last_had_finger && s_raw_log_count < 60) {
        s_raw_log_count++;
        ESP_LOGI(TAG, "RAW press #%lu x=%u y=%u",
                 (unsigned long)s_raw_log_count,
                 (unsigned)XY_Coordinate[0].x_position,
                 (unsigned)XY_Coordinate[0].y_position);
    }
    ...
    /* Diagnostic: a single read (I2C 44 B + gsl_alg_id_main) taking longer
     * than 20 ms points at I2C bus contention/stall rather than CPU. */
    const uint32_t elapsed_ms = esp_log_timestamp() - t_start_ms;
    if (elapsed_ms > 20) {
        ESP_LOGW(TAG, "touch read_data slow: %lu ms (i2c stall?)",
                 (unsigned long)elapsed_ms);
    }

    s_last_poll_ms = t_start_ms;
    s_last_had_finger = (Finger_num > 0);
...
static esp_err_t esp_lcd_touch_gsl3680_read_ram_fw(esp_lcd_touch_handle_t tp)
{
    ...
    while (retry_count < max_retries) {
        vTaskDelay(pdMS_TO_TICKS(50));  // 增加延迟到 50ms
        esp_err_t ret = touch_gsl3680_i2c_read(tp, addr, (uint8_t *)&read_buf, 4);
        
        if (ret == ESP_OK) {
            ESP_LOGI(TAG,"gsl3680 read 0xb0 = %x,%x,%x,%x (retry %d/%d)", 
            ...
            if(read_buf[3] == 0x5a && read_buf[2] == 0x5a && 
               read_buf[1] == 0x5a && read_buf[0] == 0x5a) {
                ESP_LOGI(TAG,"gsl3680 startup success");
                return ESP_OK;
            }
        } else {
            ESP_LOGW(TAG,"gsl3680 read 0xb0 failed (retry %d/%d)", 
                     retry_count + 1, max_retries);
        }
    ...
    ESP_LOGE(TAG,"gsl3680 startup failed after %d retries, last read: %x,%x,%x,%x", 
             max_retries, read_buf[3], read_buf[2], read_buf[1], read_buf[0]);
    return ESP_FAIL;
}
...
static esp_err_t esp_lcd_touch_gsl3680_load_fw(esp_lcd_touch_handle_t tp)
{
    ...
    for(source_line=0;source_line<source_len;source_line++)
    {
        ...
        esp_err_t ret;
        if(addr == 0xf0)
            ret = touch_gsl3680_i2c_write(tp,addr,wrbuf,1);
        else
            ret = touch_gsl3680_i2c_write(tp,addr,wrbuf,4);

        if (ret != ESP_OK) {
            err_count++;
            if (first_err == ESP_OK) {
                first_err = ret;
                first_err_line = source_line;
            }
        }
    }

    if (err_count > 0) {
        ESP_LOGE(TAG, "load fw: %u/%u writes FAILED; first err=%s at line %u (addr=0x%x)",
                 (unsigned)err_count, (unsigned)source_len,
                 esp_err_to_name(first_err), first_err_line, GSLX680_FW[first_err_line].offset);
    } else {
        ESP_LOGI(TAG, "load fw: all %u writes ACKed", (unsigned)source_len);
    }
