...
const SENSOR_STYLE_VARIANTS = new Set(["default", "percent", "percent_bar", "arc", "arc_semi"]);
const MONITOR_STYLE_VARIANTS = new Set([
  "default",
  "percent",
  "percent_bar",
  "arc",
...
const ARC_OPENINGS = new Set(["left", "right", "top", "bottom"]);
const MONITOR_ARC_STYLES = new Set(["arc", "arc_semi", "gauge", "gauge_needle"]);
const TEMP_STYLE_VARIANTS = new Set(["graphic", "text"]);
const ENERGY_PAGE_TYPE = "energy_dashboard";
const XIAOZHI_PAGE_TYPE = "xiaozhi";
const CAMERAS_PAGE_TYPE = "cameras";
...
const ENTITY_PICKER_CONFIGS = {
  ...
  monitor_tile: {
    domain: "sensor",
    titleKey: "entity_picker.title_monitor",
    blankKey: "entity_picker.blank_monitor",
    widgetKey: "entity_picker.widget_monitor",
    itemsKey: "entity_picker.items_sensor",
    titleFallback: "Choose Sensor",
    blankFallback: "Blank Monitor Tile",
    widgetFallback: "Monitor tile",
    itemsFallback: "sensors",
    minSearch: 2,
    liveSearch: false,
  },
  entity_list: {
    domain: "sensor",
    multi: true,
    titleKey: "entity_picker.title_entity_list",
    blankKey: "entity_picker.blank_entity_list",
    widgetKey: "entity_picker.widget_entity_list",
    itemsKey: "entity_picker.items_sensor",
    titleFallback: "Choose Entities",
    blankFallback: "Blank Entity List",
    widgetFallback: "Entity list",
    itemsFallback: "sensors",
    minSearch: 2,
    liveSearch: false,
  },
  temp_tile: {
    ...
    widgetKey: "entity_picker.widget_monitor",
    itemsKey: "entity_picker.items_sensor",
    titleFallback: "Choose Temperature Sensor",
    blankFallback: "Blank Temperature Tile",
    widgetFallback: "Temperature tile",
    itemsFallback: "sensors",
    minSearch: 2,
    liveSearch: false,
  },
...
const WEB_I18N_BUILTIN = {
  en: {
    ...
    "layout.pages.add_cameras": "+ Cameras Page",
    ...
    "layout.pages.cameras_title": "Cameras",
    ...
    "layout.status.cameras_page_only": "Camera pages are managed in Settings → Cameras and do not accept widgets.",
    "layout.status.cameras_page_locked": "This page shows live camera streams. Add and configure cameras in Settings → Cameras.",
    ...
    "layout.inspector.card_bg_color": "Card background color",
    "layout.inspector.label_color": "Label text color",
    "layout.inspector.bg_image": "Background image (SD card)",
    "layout.inspector.bg_image_upload": "Upload background image",
    "layout.inspector.bg_image_choose": "Choose from SD card",
    ...
    "setup.add_binary": "+ Binary",
    "setup.add_monitor": "+ Monitor",
    "setup.add_entity_list": "+ Entity List",
    "setup.add_sensor_tile": "+ Sensor Tile",
    "setup.add_temperature_tile": "+ Temperature",
    "setup.add_machine": "+ Machine / CPU",
    "setup.add_server": "+ Servers",
    "setup.add_network": "+ Network",
    "setup.add_glances": "+ Glances",
    "setup.close": "Close",
    "setup.skip": "Skip",
    ...
    "settings.sd.heading": "SD Card",
    "settings.sd.logging_enabled": "Write logs and sensor data to the SD card",
    "settings.sd.refresh": "Refresh status",
    "settings.sd.format": "Format as new",
    "settings.sd.format_hint": "Formatting erases all data and prepares the card (FAT32). Use it after inserting a new card.",
    "settings.sd.status_ok": "OK",
    "settings.sd.status_error": "Error",
    "settings.sd.status_missing": "No card",
    "settings.sd.status_mounted": "Mounted",
    "settings.sd.status_not_mounted": "Not mounted",
    "settings.sd.logging": "Logging: {value}",
    "settings.sd.total": "Total: {value}",
    "settings.sd.free": "Free: {value}",
    "settings.sd.format_confirm": "Format the SD card? ALL data on it will be erased.",
    "settings.sd.format_ok": "SD card formatted successfully.",
    "settings.sd.format_failed": "Format failed: {error}",
    "settings.sd.load_failed": "Failed to read SD status: {error}",
    "settings.sd.backup": "Backup config to SD",
    "settings.sd.restore": "Restore config from SD",
    "settings.sd.backup_hint": "Saves menu, entities, tiles, settings, theme and screensaver to panel-backup.json on the SD card. Restore restarts the panel.",
    "settings.sd.backup_ok": "Config backup saved to SD.",
    "settings.sd.backup_failed": "Backup failed: {error}",
    "settings.sd.restore_confirm": "Restore config from SD and restart the panel?",
    "settings.sd.restore_ok": "Config restored. Restarting...",
    "settings.sd.restore_failed": "Restore failed: {error}",
    ...
    "settings.display.dim_timeout": "Dim after inactivity",
    "settings.display.off_timeout": "Screen saver / off after inactivity",
    "settings.display.night_mode": "Night mode (screensaver appears during night hours)",
    "settings.display.night_start": "Night start hour",
    "settings.display.night_end": "Night end hour",
    "settings.display.info": "After the dim timeout the backlight dims. The screen saver appears (instead of full off) during the night window or after the off timeout. Brightness applies immediately; timeouts apply after saving.",
    "settings.display.timeout.never": "Never",
    "settings.display.timeout.1m": "1 minute",
    "settings.display.timeout.3m": "3 minutes",
    "settings.display.timeout.5m": "5 minutes",
    "settings.display.timeout.10m": "10 minutes",
    "settings.display.timeout.15m": "15 minutes",
    "settings.display.timeout.30m": "30 minutes",
    "settings.display.timeout.1h": "1 hour",
    "settings.display.timeout.2h": "2 hours",
    "settings.system.heading": "System",
    "settings.system.daily_restart": "Daily auto restart hour",
    "settings.system.daily_restart_off": "Off",
    "settings.system.audio_volume": "Audio volume (%)",
    "settings.system.restart": "Restart device",
    "settings.system.restart_confirm": "Restart the device now?",
    "settings.system.restarting": "Restarting...",
    "settings.system.info": "The daily restart hour applies after saving. Audio volume applies to the built-in speaker.",
  ...
  de: {
    ...
    "layout.status.cameras_page_only": "Kamera-Seiten werden unter Einstellungen → Kameras verwaltet und akzeptieren keine Widgets.",
    "layout.status.cameras_page_locked": "Diese Seite zeigt Live-Kamerastreams. Kameras unter Einstellungen → Kameras hinzufügen und konfigurieren.",
    "layout.widgets.heading": "Widgets",
    "layout.widgets.add_sensor": "+ Sensor",
    ...
    "layout.inspector.card_bg_color": "Karten-Hintergrundfarbe",
    "layout.inspector.label_color": "Textfarbe der Beschriftung",
    "layout.inspector.bg_image": "Hintergrundbild (SD-Karte)",
    "layout.inspector.bg_image_upload": "Hintergrundbild hochladen",
    "layout.inspector.bg_image_choose": "Von SD-Karte wählen",
    ...
    "setup.add_sensor": "+ Sensor",
    "setup.add_binary": "+ Binaer",
    "setup.add_monitor": "+ Monitor",
    "setup.add_entity_list": "+ Entitaetsliste",
    "setup.add_sensor_tile": "+ Sensor-Kachel",
    "setup.add_temperature_tile": "+ Temperatur",
    "setup.add_machine": "+ Maschine/CPU",
    "setup.add_server": "+ Server",
    "setup.add_network": "+ Netzwerk",
    "setup.add_glances": "+ Glances",
    ...
    "settings.logs.heading": "Protokolle",
    "settings.logs.refresh": "Aktualisieren",
    "settings.logs.pause": "Pause",
    "settings.logs.resume": "Fortsetzen",
    "settings.logs.clear": "Leeren",
    "settings.logs.auto_scroll": "Auto-Scroll",
    "settings.logs.download": "Protokoll herunterladen",
    "settings.logs.loading": "Protokolle werden geladen...",
    "settings.logs.empty": "Keine Einträge. Hier erscheinen Fehler, Warnungen und Absturzmarkierungen.",
    "settings.logs.updated": "Aktualisiert {time}",
    "settings.logs.fetch_failed": "Protokolle konnten nicht gelesen werden: {error}",
    "settings.logs.cleared": "Protokolldatei geleert.",
    "settings.logs.clear_failed": "Protokolle konnten nicht geleert werden: {error}",
  ...
  es: {
    ...
    "layout.inspector.card_bg_color": "Color de fondo de la tarjeta",
    "layout.inspector.label_color": "Color del texto de la etiqueta",
    "layout.inspector.bg_image": "Imagen de fondo (tarjeta SD)",
    "layout.inspector.bg_image_upload": "Subir imagen de fondo",
    "layout.inspector.bg_image_choose": "Elegir de la tarjeta SD",
    ...
    "settings.logs.heading": "Registros",
    "settings.logs.refresh": "Actualizar",
    "settings.logs.pause": "Pausar",
    "settings.logs.resume": "Reanudar",
    "settings.logs.clear": "Borrar",
    "settings.logs.auto_scroll": "Auto-scroll",
    "settings.logs.download": "Descargar registro",
    "settings.logs.loading": "Cargando registros...",
    "settings.logs.empty": "Sin entradas. Aquí aparecerán errores, avisos y marcadores de fallos.",
    "settings.logs.updated": "Actualizado {time}",
    "settings.logs.fetch_failed": "No se pudieron leer los registros: {error}",
    "settings.logs.cleared": "Archivo de registro borrado.",
    "settings.logs.clear_failed": "No se pudieron borrar los registros: {error}",
  ...
  fr: {
    ...
    "layout.inspector.card_bg_color": "Couleur de fond de la carte",
    "layout.inspector.label_color": "Couleur du texte de l'etiquette",
    "layout.inspector.bg_image": "Image de fond (carte SD)",
    "layout.inspector.bg_image_upload": "Téléverser une image de fond",
    "layout.inspector.bg_image_choose": "Choisir depuis la carte SD",
    ...
    "settings.logs.heading": "Journaux",
    "settings.logs.refresh": "Actualiser",
    "settings.logs.pause": "Pause",
    "settings.logs.resume": "Reprendre",
    "settings.logs.clear": "Effacer",
    "settings.logs.auto_scroll": "Défilement auto",
    "settings.logs.download": "Télécharger le journal",
    "settings.logs.loading": "Chargement des journaux...",
    "settings.logs.empty": "Aucune entrée. Les erreurs, avertissements et marqueurs de crash apparaîtront ici.",
    "settings.logs.updated": "Mis à jour {time}",
    "settings.logs.fetch_failed": "Impossible de lire les journaux : {error}",
    "settings.logs.cleared": "Fichier journal effacé.",
    "settings.logs.clear_failed": "Impossible d'effacer les journaux : {error}",
  ...
  pl: {
    ...
    "layout.pages.add_cameras": "+ Strona Kamer",
    ...
    "layout.pages.xiaozhi_title": "Xiaozhi",
    "layout.pages.cameras_title": "Kamery",
    ...
    "layout.status.cameras_page_only": "Strony Kamer są zarządzane w Ustawienia → Kamery i nie przyjmują widżetów.",
    "layout.status.cameras_page_locked": "Ta strona pokazuje podgląd kamer na żywo. Dodawaj i konfiguruj kamery w Ustawienia → Kamery.",
    ...
    "layout.inspector.card_bg_color": "Kolor tła kafelka",
    "layout.inspector.label_color": "Kolor tekstu etykiety",
    "layout.inspector.bg_image": "Obraz tła (karta SD)",
    "layout.inspector.bg_image_upload": "Wgraj obraz tła",
    "layout.inspector.bg_image_choose": "Wybierz z karty SD",
    ...
    "setup.add_binary": "+ Binarny",
    "setup.add_monitor": "+ Monitor",
    "setup.add_entity_list": "+ Lista encji",
    "setup.add_sensor_tile": "+ Kafelek czujnika",
    "setup.add_temperature_tile": "+ Temperatura",
    "setup.add_machine": "+ Maszyna/CPU",
    "setup.add_server": "+ Serwery",
    "setup.add_network": "+ Sieć",
    "setup.add_glances": "+ Glances",
    "setup.close": "Zamknij",
    "setup.skip": "Pomiń",
    ...
    "settings.sd.heading": "Karta SD",
    "settings.sd.logging_enabled": "Zapisuj logi i dane czujników na karcie SD",
    "settings.sd.refresh": "Odśwież status",
    "settings.sd.format": "Formatuj jako nową",
    "settings.sd.format_hint": "Formatowanie usuwa wszystkie dane i przygotowuje kartę (FAT32). Użyj po włożeniu nowej karty.",
    "settings.sd.status_ok": "OK",
    "settings.sd.status_error": "Błąd",
    "settings.sd.status_missing": "Brak karty",
    "settings.sd.status_mounted": "Zamontowana",
    "settings.sd.status_not_mounted": "Nie zamontowana",
    "settings.sd.logging": "Zapis: {value}",
    "settings.sd.total": "Pojemność: {value}",
    "settings.sd.free": "Wolne: {value}",
    "settings.sd.format_confirm": "Sformatować kartę SD? WSZYSTKIE dane na niej zostaną usunięte.",
    "settings.sd.format_ok": "Karta SD sformatowana pomyślnie.",
    "settings.sd.format_failed": "Formatowanie nie powiodło się: {error}",
    "settings.sd.load_failed": "Nie udało się odczytać statusu karty SD: {error}",
    "settings.sd.backup": "Zapisz kopię konfiguracji na SD",
    "settings.sd.restore": "Przywróć konfigurację z SD",
    "settings.sd.backup_hint": "Zapisuje menu, encje, kafelki, ustawienia, motyw i wygaszacz do panel-backup.json na karcie SD. Przywracanie restartuje panel.",
    "settings.sd.backup_ok": "Kopia konfiguracji zapisana na SD.",
    "settings.sd.backup_failed": "Kopia nie powiodła się: {error}",
    "settings.sd.restore_confirm": "Przywrócić konfigurację z SD i zrestartować panel?",
    "settings.sd.restore_ok": "Konfiguracja przywrócona. Restartowanie...",
    "settings.sd.restore_failed": "Przywracanie nie powiodło się: {error}",
    ...
    "settings.display.dim_timeout": "Przyciemnij po bezczynności",
    "settings.display.off_timeout": "Wygaszacz / wyłącz po bezczynności",
    "settings.display.night_mode": "Tryb nocny (wygaszacz pojawia się w godzinach nocnych)",
    "settings.display.night_start": "Godzina rozpoczęcia nocy",
    "settings.display.night_end": "Godzina zakończenia nocy",
    "settings.display.info": "Po upływie czasu przyciemnienia ekran jest przyciemniany. Wygaszacz pojawia się (zamiast pełnego wyłączenia) w oknie nocnym lub po upływie czasu wyłączenia. Jasność działa natychmiast; czasy działają po zapisaniu.",
    "settings.display.timeout.never": "Nigdy",
    "settings.display.timeout.1m": "1 minuta",
    "settings.display.timeout.3m": "3 minuty",
    "settings.display.timeout.5m": "5 minut",
    "settings.display.timeout.10m": "10 minut",
    "settings.display.timeout.15m": "15 minut",
    "settings.display.timeout.30m": "30 minut",
    "settings.display.timeout.1h": "1 godzina",
    "settings.display.timeout.2h": "2 godziny",
    "settings.system.heading": "System",
    "settings.system.daily_restart": "Godzina codziennego restartu",
    "settings.system.daily_restart_off": "Wyłączony",
    "settings.system.audio_volume": "Głośność (%)",
    "settings.system.restart": "Uruchom ponownie",
    "settings.system.restart_confirm": "Uruchomić ponownie urządzenie teraz?",
    "settings.system.restarting": "Uruchamianie ponownie...",
    "settings.system.info": "Godzina codziennego restartu działa po zapisaniu. Głośność dotyczy wbudowanego głośnika.",
...
function widgetSizeLimits(type) {
  ...
  switch (type) {
    ...
    case "monitor_tile":
      return compact
        ? { minW: 170, minH: 130, maxW: CANVAS_WIDTH, maxH: CANVAS_HEIGHT }
        : { minW: 240, minH: 180, maxW: CANVAS_WIDTH, maxH: CANVAS_HEIGHT };
    case "temp_tile":
      return compact
        ? { minW: 130, minH: 140, maxW: CANVAS_WIDTH, maxH: CANVAS_HEIGHT }
        : { minW: 160, minH: 170, maxW: CANVAS_WIDTH, maxH: CANVAS_HEIGHT };
    case "entity_list":
    case "sensor_tile":
      return compact
        ? { minW: 190, minH: 160, maxW: CANVAS_WIDTH, maxH: CANVAS_HEIGHT }
        : { minW: 240, minH: 200, maxW: CANVAS_WIDTH, maxH: CANVAS_HEIGHT };
    default:
      return fallback;
  }
}
...
const editor = {
  ...
  appScreenW: 0,
  appScreenH: 0,
  features: { cameras: true, local_camera: false, xiaozhi: true },
...
const el = {
  ...
  addCamerasPageBtn: document.getElementById("addCamerasPageBtn"),
  ...
  lightEntityPickerConfirmBtn: document.getElementById("lightEntityPickerConfirmBtn"),
  ...
  addMonitorTileBtn: document.getElementById("addMonitorTileBtn"),
  addEntityListBtn: document.getElementById("addEntityListBtn"),
  addSensorTileBtn: document.getElementById("addSensorTileBtn"),
  addTemperatureTileBtn: document.getElementById("addTemperatureTileBtn"),
  ...
  cardStyleOptions: document.getElementById("cardStyleOptions"),
  fCardBgColor: document.getElementById("fCardBgColor"),
  fLabelColor: document.getElementById("fLabelColor"),
  fBgImage: document.getElementById("fBgImage"),
  fBgImageFile: document.getElementById("fBgImageFile"),
  fBgImageUploadBtn: document.getElementById("fBgImageUploadBtn"),
  fBgImageSelect: document.getElementById("fBgImageSelect"),
  fBgImageRefreshBtn: document.getElementById("fBgImageRefreshBtn"),
  sdListRefreshBtn: document.getElementById("sdListRefreshBtn"),
  sdFileList: document.getElementById("sdFileList"),
  sdFilePreviewImg: document.getElementById("sdFilePreviewImg"),
  sdFilePreviewText: document.getElementById("sdFilePreviewText"),
  sdFilePreviewPath: document.getElementById("sdFilePreviewPath"),
  sdFilePreviewEmpty: document.getElementById("sdFilePreviewEmpty"),
  ...
  fSensorStyleVariant: document.getElementById("fSensorStyleVariant"),
  fSensorArcOpening: document.getElementById("fSensorArcOpening"),
  fSensorArcOpeningWrap: document.getElementById("fSensorArcOpeningWrap"),
  fSensorMin: document.getElementById("fSensorMin"),
  fSensorMax: document.getElementById("fSensorMax"),
  monitorOptions: document.getElementById("monitorOptions"),
  fMonitorStyleVariant: document.getElementById("fMonitorStyleVariant"),
  fMonitorArcOpening: document.getElementById("fMonitorArcOpening"),
  fMonitorArcOpeningWrap: document.getElementById("fMonitorArcOpeningWrap"),
  fMonitorMin: document.getElementById("fMonitorMin"),
  fMonitorMax: document.getElementById("fMonitorMax"),
  fMonitorSubEntities: document.getElementById("fMonitorSubEntities"),
  tempOptions: document.getElementById("tempOptions"),
  fTempStyleVariant: document.getElementById("fTempStyleVariant"),
  fTempMin: document.getElementById("fTempMin"),
  fTempMax: document.getElementById("fTempMax"),
  entityListOptions: document.getElementById("entityListOptions"),
  fEntityListIds: document.getElementById("fEntityListIds"),
  sensorTileOptions: document.getElementById("sensorTileOptions"),
  fSensorTileIds: document.getElementById("fSensorTileIds"),
  sensorTileRowsEditor: document.getElementById("sensorTileRowsEditor"),
  sensorTileAddRowBtn: document.getElementById("sensorTileAddRowBtn"),
  fSensorTileTitleFont: document.getElementById("fSensorTileTitleFont"),
  fSensorTileRowFont: document.getElementById("fSensorTileRowFont"),
  fSensorTileIpFont: document.getElementById("fSensorTileIpFont"),
  fSensorTilePowerFont: document.getElementById("fSensorTilePowerFont"),
  fSensorTilePortsFont: document.getElementById("fSensorTilePortsFont"),
  fSensorTileLan: document.getElementById("fSensorTileLan"),
  fSensorTileWan: document.getElementById("fSensorTileWan"),
  fSensorTileTcp: document.getElementById("fSensorTileTcp"),
  fSensorTileUdp: document.getElementById("fSensorTileUdp"),
  fSensorTilePower: document.getElementById("fSensorTilePower"),
  ...
  settingsDimBrightness: document.getElementById("settingsDimBrightness"),
  settingsDimTimeout: document.getElementById("settingsDimTimeout"),
  settingsOffTimeout: document.getElementById("settingsOffTimeout"),
  ...
  settingsSdLoggingEnabled: document.getElementById("settingsSdLoggingEnabled"),
  settingsSdInfo: document.getElementById("settingsSdInfo"),
  refreshSdStatusBtn: document.getElementById("refreshSdStatusBtn"),
  formatSdBtn: document.getElementById("formatSdBtn"),
  ...
  setupWizardAddBinaryBtn: document.getElementById("setupWizardAddBinaryBtn"),
  setupWizardAddMonitorBtn: document.getElementById("setupWizardAddMonitorBtn"),
  setupWizardAddEntityListBtn: document.getElementById("setupWizardAddEntityListBtn"),
  setupWizardAddSensorTileBtn: document.getElementById("setupWizardAddSensorTileBtn"),
  setupWizardAddTemperatureTileBtn: document.getElementById("setupWizardAddTemperatureTileBtn"),
  setupWizardAddMachineBtn: document.getElementById("setupWizardAddMachineBtn"),
  setupWizardAddServerBtn: document.getElementById("setupWizardAddServerBtn"),
  setupWizardAddNetworkBtn: document.getElementById("setupWizardAddNetworkBtn"),
  setupWizardAddGlancesBtn: document.getElementById("setupWizardAddGlancesBtn"),
  ...
  for (const page of layout.pages) {
    ...
    for (const widget of page.widgets) {
      ...
      if (widget.type === "sensor") {
        if (!SENSOR_STYLE_VARIANTS.has(widget.style_variant)) {
          widget.style_variant = "default";
        }
        if (widget.style_variant !== "arc" && widget.style_variant !== "arc_semi") {
          delete widget.arc_opening;
        } else if (!ARC_OPENINGS.has(widget.arc_opening)) {
          widget.arc_opening = "left";
        }
        if (typeof widget.sensor_min !== "number") delete widget.sensor_min;
        if (typeof widget.sensor_max !== "number") delete widget.sensor_max;
      }
      if (widget.type === "monitor_tile") {
        if (!MONITOR_STYLE_VARIANTS.has(widget.style_variant)) {
          widget.style_variant = "default";
        }
        if (!MONITOR_ARC_STYLES.has(widget.style_variant)) {
          delete widget.arc_opening;
        } else if (!ARC_OPENINGS.has(widget.arc_opening)) {
          widget.arc_opening = "left";
        }
        if (typeof widget.sensor_min !== "number") delete widget.sensor_min;
        if (typeof widget.sensor_max !== "number") delete widget.sensor_max;
        widget.sub_entity_ids = normalizeEntityIdList(widget.sub_entity_ids).slice(0, 4).join(", ");
        if (!widget.sub_entity_ids) delete widget.sub_entity_ids;
      } else if (widget.type === "temp_tile") {
        if (!TEMP_STYLE_VARIANTS.has(widget.style_variant)) {
          widget.style_variant = "graphic";
        }
        delete widget.arc_opening;
        if (typeof widget.sensor_min !== "number") delete widget.sensor_min;
        if (typeof widget.sensor_max !== "number") delete widget.sensor_max;
        delete widget.sub_entity_ids;
      } else if (widget.type !== "sensor" && widget.type !== "heating_tile" && widget.type !== "button") {
        delete widget.sub_entity_ids;
      }
      if (widget.type === "entity_list") {
        widget.entity_ids = normalizeEntityIdList(widget.entity_ids).slice(0, 24).join(", ");
        delete widget.entity_id;
        delete widget.style_variant;
        delete widget.arc_opening;
      } else if (widget.type === "sensor_tile") {
        widget.entity_ids = normalizeSensorTileRows(widget.entity_ids).join(", ");
        delete widget.entity_id;
        delete widget.style_variant;
        delete widget.arc_opening;
      } else {
...
async function loadAppVersion() {
  try {
    ...
    if (payload && typeof payload === "object") {
      if (typeof payload.feature_cameras === "boolean") editor.features.cameras = payload.feature_cameras;
      if (typeof payload.feature_local_camera === "boolean") editor.features.local_camera = payload.feature_local_camera;
      if (typeof payload.feature_xiaozhi === "boolean") editor.features.xiaozhi = payload.feature_xiaozhi;
    }
  ...
  applySettingsFeatureVisibility();
  renderAppVersion();
}
...
function applyWebTranslations() {
  ...
  setTextById("addCamerasPageBtn", "layout.pages.add_cameras");
  ...
  setTextById("fCardBgColorLabel", "layout.inspector.card_bg_color");
  setTextById("fLabelColorLabel", "layout.inspector.label_color");
  setTextById("fBgImageLabel", "layout.inspector.bg_image");
  setTextById("fBgImageUploadLabel", "layout.inspector.bg_image_upload");
  setTextById("fBgImageSelectLabel", "layout.inspector.bg_image_choose");
  ...
  setTextById("settingsSdLoggingEnabledLabel", "settings.sd.logging_enabled");
  setTextById("refreshSdStatusBtn", "settings.sd.refresh");
  setTextById("formatSdBtn", "settings.sd.format");
  setTextById("settingsSdFormatHint", "settings.sd.format_hint");
  setTextById("backupConfigBtn", "settings.sd.backup");
  setTextById("restoreConfigBtn", "settings.sd.restore");
  setTextById("settingsSdBackupHint", "settings.sd.backup_hint");
  ...
  setTextById("settingsTouchTestLabel", "settings.ui.touch_test");
  ...
  setTextById("settingsDimBrightnessLabel", "settings.display.dim_brightness");
  setTextById("settingsDimTimeoutLabel", "settings.display.dim_timeout");
  setTextById("settingsOffTimeoutLabel", "settings.display.off_timeout");
  setTextById("settingsNightModeEnabledLabel", "settings.display.night_mode");
  ...
  setTextById("settingsDisplayInfo", "settings.display.info");
  populateDisplayTimeouts();
  populateNightHours();

  setTextById("settingsSystemHeading", "settings.system.heading");
  setTextById("settingsDailyRestartLabel", "settings.system.daily_restart");
  setTextById("settingsAudioVolumeLabel", "settings.system.audio_volume");
  setTextById("restartDeviceBtn", "settings.system.restart");
  setTextById("settingsSystemInfo", "settings.system.info");
  ...
  setTextById("setupWizardAddBinaryBtn", "setup.add_binary");
  setTextById("setupWizardAddMonitorBtn", "setup.add_monitor");
  setTextById("setupWizardAddEntityListBtn", "setup.add_entity_list");
  setTextById("setupWizardAddSensorTileBtn", "setup.add_sensor_tile");
  setTextById("setupWizardAddTemperatureTileBtn", "setup.add_temperature_tile");
  setTextById("setupWizardAddMachineBtn", "setup.add_machine");
  setTextById("setupWizardAddServerBtn", "setup.add_server");
  setTextById("setupWizardAddNetworkBtn", "setup.add_network");
  setTextById("setupWizardAddGlancesBtn", "setup.add_glances");
...
function settingsFeatureHidden(sectionId) {
  const item = SETTINGS_NAV_ITEMS.find((it) => it.sectionId === sectionId);
  if (!item || !item.feature) return false;
  return editor.features?.[item.feature] === false;
}

function applySettingsFeatureVisibility() {
  for (const button of el.settingsNavButtons || []) {
    button.classList.toggle("hidden", settingsFeatureHidden(button.dataset.settingsSection));
  }
  const visibleItems = SETTINGS_NAV_ITEMS.filter((it) => !settingsFeatureHidden(it.sectionId));
  const activeVisible = visibleItems.some((it) => it.sectionId === editor.activeSettingsSection);
  if (!activeVisible && visibleItems.length > 0) {
    editor.activeSettingsSection = visibleItems[0].sectionId;
  }
  setActiveSettingsSection(editor.activeSettingsSection);
}
...
function setActiveSettingsSection(sectionId) {
  ...
  for (const section of el.settingsContentSections || []) {
    const hidden = section.id !== item.sectionId || settingsFeatureHidden(section.id);
...
const DISPLAY_DIM_TIMEOUTS = [
  [0, "settings.display.timeout.never"],
  [60000, "settings.display.timeout.1m"],
  [180000, "settings.display.timeout.3m"],
  [300000, "settings.display.timeout.5m"],
  [600000, "settings.display.timeout.10m"],
  [900000, "settings.display.timeout.15m"],
  [1800000, "settings.display.timeout.30m"],
  [3600000, "settings.display.timeout.1h"],
];

const DISPLAY_OFF_TIMEOUTS = [
  [0, "settings.display.timeout.never"],
  [60000, "settings.display.timeout.1m"],
  [300000, "settings.display.timeout.5m"],
  [600000, "settings.display.timeout.10m"],
  ...
  [1800000, "settings.display.timeout.30m"],
  [3600000, "settings.display.timeout.1h"],
  [7200000, "settings.display.timeout.2h"],
];

function settingsInt(raw, min, max, fallback) {
  const n = Number(raw);
  if (!Number.isFinite(n)) return fallback;
  return Math.round(clamp(n, min, max));
}

function fillTimeoutSelect(select, list, selectedMs) {
  if (!select) return;
  select.innerHTML = "";
  for (const [ms, key] of list) {
    const opt = document.createElement("option");
    opt.value = String(ms);
    opt.textContent = t(key);
    select.appendChild(opt);
  }
  select.value = String(selectedMs);
}

function populateDisplayTimeouts() {
  fillTimeoutSelect(el.settingsDimTimeout, DISPLAY_DIM_TIMEOUTS, 180000);
  fillTimeoutSelect(el.settingsOffTimeout, DISPLAY_OFF_TIMEOUTS, 1800000);
}

function fillHourSelect(select, selectedHour) {
  if (!select) return;
  select.innerHTML = "";
  for (let h = 0; h < 24; h++) {
    const opt = document.createElement("option");
...
function populateNightHours() {
  fillHourSelect(el.settingsNightStart, 22);
  fillHourSelect(el.settingsNightEnd, 6);
}

function fillDailyRestartSelect(select, selectedHour) {
  if (!select) return;
  select.innerHTML = "";
  const off = document.createElement("option");
  off.value = "-1";
  off.textContent = t("settings.system.daily_restart_off");
...
function populateDailyRestartHour() {
  const current = el.settingsDailyRestartHour?.value;
  const selected = current === undefined || current === "" ? -1 : Number(current);
  fillDailyRestartSelect(el.settingsDailyRestartHour, selected);
}
...
function renderSettings() {
  ...
  if (el.settingsDailyRestartHour) {
    populateDailyRestartHour();
    el.settingsDailyRestartHour.value = String(settingsInt(system.daily_restart_hour, -1, 23, -1));
  }
  if (el.settingsAudioVolume) {
    el.settingsAudioVolume.value = settingsInt(settings.audio?.volume, 0, 100, 70);
  }
  ...
  if (el.settingsActiveBrightness) {
    el.settingsActiveBrightness.value = settingsInt(display.active_brightness, 0, 100, 100);
  }
  if (el.settingsDimBrightness) {
    el.settingsDimBrightness.value = settingsInt(display.dim_brightness, 0, 100, 10);
  }
  fillTimeoutSelect(el.settingsDimTimeout, DISPLAY_DIM_TIMEOUTS,
    settingsInt(display.dim_timeout_ms, 0, 86400000, 180000));
  fillTimeoutSelect(el.settingsOffTimeout, DISPLAY_OFF_TIMEOUTS,
    settingsInt(display.off_timeout_ms, 0, 86400000, 1800000));
  if (el.settingsNightModeEnabled) {
    el.settingsNightModeEnabled.checked = display.night_mode_enabled === true;
  }
  fillHourSelect(el.settingsNightStart, settingsInt(display.night_start_hour, 0, 23, 22));
  fillHourSelect(el.settingsNightEnd, settingsInt(display.night_end_hour, 0, 23, 6));
  renderLanguageOptions();
...
function renderSdStatus(status) {
  ...
  if (!status || !status.ok) {
    el.settingsSdInfo.textContent = t("settings.sd.status_error");
    return;
  }
  const mounted = status.mounted === true;
  const lines = [];
  lines.push(`${t("settings.sd.status_mounted")}: ${mounted ? t("common.yes") : t("common.no")}`);
  lines.push(`${t("settings.sd.logging", { value: status.logging_enabled ? t("common.yes") : t("common.no") })}`);
  if (mounted && Number.isFinite(Number(status.total_bytes))) {
    lines.push(`${t("settings.sd.total", { value: formatGigaBytes(status.total_bytes) })}`);
    lines.push(`${t("settings.sd.free", { value: formatGigaBytes(status.free_bytes) })}`);
  }
  el.settingsSdInfo.textContent = lines.join(" | ");
}

async function loadSdStatus() {
  try {
    const status = await apiGet("/api/sd/status");
    renderSdStatus(status);
    if (status && !status.ok) {
      setStatus(t("settings.sd.status_error"), true);
    }
    return status;
  } catch (err) {
    if (el.settingsSdInfo) {
      el.settingsSdInfo.textContent = t("settings.sd.load_failed", { error: err.message });
    }
    setStatus(t("settings.sd.load_failed", { error: err.message }), true);
    return null;
  }
}

async function formatSdCard() {
  const confirmed = window.confirm(t("settings.sd.format_confirm"));
  if (!confirmed) return;
  setStatus(t("settings.sd.format_confirm"));
  try {
    const response = await fetch("/api/sd/format", {
      method: "POST",
      headers: { "Content-Type": "application/json" },
    ...
    setStatus(t("settings.sd.format_ok"));
    await loadSdStatus();
  } catch (err) {
    setStatus(t("settings.sd.format_failed", { error: err.message }), true);
  }
}

async function backupConfig() {
  setStatus(t("settings.sd.backup"));
  try {
    ...
    setStatus(t("settings.sd.backup_ok"));
    await loadSdStatus();
  } catch (err) {
    setStatus(t("settings.sd.backup_failed", { error: err.message }), true);
  }
}

async function restoreConfig() {
  const confirmed = window.confirm(t("settings.sd.restore_confirm"));
  if (!confirmed) return;
  try {
    ...
    setStatus(t("settings.sd.restore_ok"));
  } catch (err) {
    setStatus(t("settings.sd.restore_failed", { error: err.message }), true);
  }
}

async function restartDevice() {
  const confirmed = window.confirm(t("settings.system.restart_confirm"));
  if (!confirmed) return;
  setStatus(t("settings.system.restarting"));
  try {
    await putSettings({ reboot: true });
  } catch (err) {
...
function sanitizeBgFileName(name) {
  const base = (name || "").split(/[\\/]/).pop() || "tile.png";
  const m = base.toLowerCase().match(/\.(png|jpe?g)$/);
  const ext = m ? (m[1] === "jpeg" ? "jpg" : m[1]) : "png";
  let stem = base.replace(/\.[^.]*$/, "");
  stem = stem.replace(/[^a-zA-Z0-9_-]/g, "_").replace(/^_+|_+$/g, "");
  if (!stem) {
    stem = "tile_" + Date.now().toString(36);
  }
  return stem + "." + ext;
}

async function uploadTileBackground() {
  const file = el.fBgImageFile?.files?.[0];
  if (!file) {
    setStatus(t("layout.inspector.bg_image_upload") + ": " + t("settings.ota.no_file"), true);
    return;
  }

  const name = sanitizeBgFileName(file.name);
  const url = "/api/sd/bg/upload?name=" + encodeURIComponent(name);
  try {
    const payload = await new Promise((resolve, reject) => {
      const xhr = new XMLHttpRequest();
      xhr.open("POST", url);
    ...
    setStatus(t("layout.inspector.bg_image_upload") + ": " + (payload.name || "ok"), false);
    if (el.fBgImage) {
      el.fBgImage.value = payload.path || "";
    }
    if (el.fBgImageFile) {
      el.fBgImageFile.value = "";
    }
    await loadTileBackgroundList(payload.path);
...
async function loadTileBackgroundList(selectedPath) {
  const sel = el.fBgImageSelect;
  if (!sel) {
    return;
  }
  let files = [];
  ...
  const target = selectedPath || el.fBgImage?.value || "";
  if (target) {
    let found = false;
    for (const opt of sel.options) {
      if (opt.value === target) {
...
function resetSdFilePreview() {
  if (el.sdFilePreviewImg) {
    el.sdFilePreviewImg.hidden = true;
    el.sdFilePreviewImg.removeAttribute("src");
  }
  if (el.sdFilePreviewText) {
    el.sdFilePreviewText.hidden = true;
    el.sdFilePreviewText.textContent = "";
  }
  if (el.sdFilePreviewPath) {
    el.sdFilePreviewPath.textContent = "";
  }
  if (el.sdFilePreviewEmpty) {
    el.sdFilePreviewEmpty.hidden = false;
  }
}
...
function isPreviewableText(name) {
  return /\.(txt|log|json|md|cfg|ini|csv)$/i.test(name || "");
}

async function previewSdFile(path) {
  resetSdFilePreview();
  if (!path) {
    return;
  }
  ...
  if (el.sdFilePreviewEmpty) {
    el.sdFilePreviewEmpty.hidden = true;
  }
  if (el.sdFilePreviewPath) {
    el.sdFilePreviewPath.textContent = path;
  }

  const url = "/api/sd/file?path=" + encodeURIComponent(path);
  if (isPreviewableImage(path)) {
    if (el.sdFilePreviewImg) {
      el.sdFilePreviewImg.src = url;
      el.sdFilePreviewImg.hidden = false;
    }
    return;
  }

  if (isPreviewableText(path)) {
    try {
      ...
      if (el.sdFilePreviewText) {
        el.sdFilePreviewText.textContent = text.slice(0, 65536);
        el.sdFilePreviewText.hidden = false;
      }
    } catch (err) {
      if (el.sdFilePreviewEmpty) {
        el.sdFilePreviewEmpty.textContent = "Preview failed: " + err.message;
        el.sdFilePreviewEmpty.hidden = false;
      }
    }
    return;
  ...
  if (el.sdFilePreviewEmpty) {
    el.sdFilePreviewEmpty.textContent = "No preview for this file type.";
    el.sdFilePreviewEmpty.hidden = false;
  }
}

async function loadSdFileList() {
  const list = el.sdFileList;
  if (!list) {
    return;
  }
  ...
  let files = [];
  try {
    const resp = await fetch("/api/sd/list");
    const data = await resp.json();
    files = Array.isArray(data.files) ? data.files : [];
  } catch (err) {
    files = [];
  ...
  if (files.length === 0) {
    ...
    none.className = "meta";
    none.textContent = "No files found on the SD card.";
  ...
  const sorted = [...files].sort((a, b) => {
    ...
    return String(a.rel || a.name).localeCompare(String(b.rel || b.name));
  });

  for (const f of sorted) {
    const item = document.createElement("button");
    item.type = "button";
    item.className = "sd-file-item" + (f.is_dir ? " is-dir" : "");
    const icon = f.is_dir ? "▸ " : "";
    item.textContent = `${icon}${f.rel || f.name} (${formatBytes(f.size || 0)})`;
    item.title = f.path || "";
    if (!f.is_dir) {
      item.onclick = () => previewSdFile(f.path);
    }
...
function renderCamerasList() {
  ...
  list.forEach((cam, index) => {
    ...
    const detail = cam.source === "ha" ? (cam.entity_id || "") : (cam.snapshot_url || "");
    button.textContent = `${cam.name || detail || `Kamera ${index + 1}`}${cam.enabled === false ? " (off)" : ""}`;
...
async function saveSettings() {
  ...
  const sdLoggingEnabled = Boolean(el.settingsSdLoggingEnabled?.checked ?? true);
  ...
  const dimTimeoutMs = settingsInt(el.settingsDimTimeout?.value, 0, 86400000, 180000);
  const offTimeoutMs = settingsInt(el.settingsOffTimeout?.value, 0, 86400000, 1800000);
  const nightModeEnabled = Boolean(el.settingsNightModeEnabled?.checked);
  const nightStartHour = settingsInt(el.settingsNightStart?.value, 0, 23, 22);
  const nightEndHour = settingsInt(el.settingsNightEnd?.value, 0, 23, 6);
  ...
  const payload = {
    ...
    sd: {
      logging_enabled: sdLoggingEnabled,
    },
    time: {
    ...
    system: {
      ...
      daily_restart_hour: dailyRestartHour,
    },
    audio: {
      volume: audioVolume,
    },
...
function isCamerasPage(page) {
  return page?.type === CAMERAS_PAGE_TYPE;
}

function pageAcceptsWidgets(page) {
  return !isEnergyPage(page) && !isXiaozhiPage(page) && !isCamerasPage(page);
}
...
function allowedEntityDomainsForWidgetType(
  ...
  if (type === "temp_tile") return ["sensor"];
  if (type === "monitor_tile") return ["sensor", "binary_sensor"];
  if (type === "entity_list" || type === "sensor_tile") return [];
...
function listEntitiesForWidgetType(
  ...
  if (type === "empty_tile" || type === "entity_list" || type === "sensor_tile") return [];
...
function updateSensorArcOpeningWrapVisibility() {
  if (el.fSensorArcOpeningWrap) {
    const sensorArc = el.fSensorStyleVariant?.value === "arc" || el.fSensorStyleVariant?.value === "arc_semi";
    el.fSensorArcOpeningWrap.classList.toggle("hidden", !sensorArc);
  }
  if (el.fMonitorArcOpeningWrap) {
    const monitorArc = MONITOR_ARC_STYLES.has(el.fMonitorStyleVariant?.value);
    el.fMonitorArcOpeningWrap.classList.toggle("hidden", !monitorArc);
  }
}

function updateLightPickerConfirmText() {
  if (!el.lightEntityPickerConfirmBtn) return;
  const count = editor.lightPicker.selected.length;
  el.lightEntityPickerConfirmBtn.textContent = count > 0
    ? t("entity_picker.add_selected", { count }, `Add ${count} selected`)
    : t("entity_picker.add_selected_none", {}, "Add selected");
  el.lightEntityPickerConfirmBtn.disabled = count === 0;
}
...
function renderLightEntityPicker(data = {}) {
  ...
  for (const group of groups) {
    ...
    for (const item of group.entries) {
      ...
      if (config.multi) {
        const selected = editor.lightPicker.selected.includes(item.id);
        button.classList.toggle("light-picker-entity-selected", selected);
        button.onclick = () => {
          const index = editor.lightPicker.selected.indexOf(item.id);
          if (index >= 0) {
            editor.lightPicker.selected.splice(index, 1);
            button.classList.remove("light-picker-entity-selected");
          } else {
            editor.lightPicker.selected.push(item.id);
            button.classList.add("light-picker-entity-selected");
          }
          updateLightPickerConfirmText();
        };
      } else {
...
function openLightEntityPicker(widgetType = "light_tile") {
  ...
  if (el.lightEntityPickerConfirmBtn) {
    el.lightEntityPickerConfirmBtn.classList.toggle("hidden", !config.multi);
  }
  renderLightEntityPicker({
...
function renderEntityOptions() {
  ...
  const primaryEnabled = inspectorType !== "empty_tile" && inspectorType !== "entity_list" && inspectorType !== "sensor_tile";
  if (el.fEntityWrap) {
...
function renderPages() {
  ...
  for (const page of editor.layout.pages) {
    ...
    const label = document.createElement("span");
    const badge = isEnergyPage(page) ? " ⚡" : isXiaozhiPage(page) ? " 🎤" : isCamerasPage(page) ? " 📷" : "";
    label.textContent = `${page.title || page.id}${badge}`;
    label.title = `[${page.id}] ${isEnergyPage(page) ? "energy" : isXiaozhiPage(page) ? "xiaozhi" : isCamerasPage(page) ? "cameras" : "page"}`;
...
function renderPagesMini() {
  ...
  for (const page of editor.layout.pages) {
    ...
    const prefix = isEnergyPage(page) ? "E " : isXiaozhiPage(page) ? "X " : isCamerasPage(page) ? "C " : "";
...
function renderWidgets() {
  ...
  const addButtons = [
    ...
    el.addSelectBtn,
    el.addNumberBtn,
    el.addMonitorTileBtn,
    el.addEntityListBtn,
    el.addSensorTileBtn,
    el.addTemperatureTileBtn,
  ];
  ...
  if (lockedPage) {
    ...
    li.textContent = t(isXiaozhiPage(page)
      ? "layout.status.xiaozhi_page_only"
      : isCamerasPage(page)
        ? "layout.status.cameras_page_only"
...
function renderCanvas() {
  ...
  if (isCamerasPage(page)) {
    const note = document.createElement("div");
    note.className = "canvas-empty-note";
    note.textContent = t("layout.status.cameras_page_locked");
    el.canvas.appendChild(note);
    return;
  }
  ...
  for (const widget of page.widgets) {
    ...
    if (widget.type === "monitor_tile") {
      const variant = MONITOR_STYLE_VARIANTS.has(widget.style_variant) ? widget.style_variant : "default";
      const subs = normalizeEntityIdList(widget.sub_entity_ids);
      extraHint = `<div class="w-hint">style ${variant}${subs.length ? ` · ${subs.length} sub` : ""}</div>`;
    }
    if (widget.type === "entity_list") {
      const rows = normalizeEntityIdList(widget.entity_ids).length;
      extraHint = `<div class="w-hint">${rows}/24 entities</div>`;
    }
    if (widget.type === "sensor_tile") {
      const rows = normalizeSensorTileRows(widget.entity_ids).length;
      extraHint = `<div class="w-hint">${rows}/24 sensors</div>`;
    }
    if (widget.type === "temp_tile") {
      const variant = TEMP_STYLE_VARIANTS.has(widget.style_variant) ? widget.style_variant : "graphic";
      extraHint = `<div class="w-hint">style ${variant}</div>`;
    }
    box.innerHTML = `
...
function renderInspector() {
  ...
  if (!widget) {
    ...
    el.fSecondaryEntity.value = "";
    if (el.fCardBgColor) {
      el.fCardBgColor.value = "";
    }
    if (el.fLabelColor) {
      el.fLabelColor.value = "";
    }
    el.fX.value = "";
    el.fY.value = "";
    el.fW.value = "";
    ...
    if (el.fSensorStyleVariant) {
      el.fSensorStyleVariant.value = "default";
    }
    if (el.fSensorArcOpening) {
      el.fSensorArcOpening.value = "left";
    }
    if (el.fSensorArcOpeningWrap) {
      el.fSensorArcOpeningWrap.classList.add("hidden");
    }
    if (el.fSensorMin) {
      el.fSensorMin.value = "";
    }
    ...
    if (el.monitorOptions) {
      el.monitorOptions.classList.add("hidden");
    }
    if (el.fMonitorStyleVariant) {
      el.fMonitorStyleVariant.value = "default";
    }
    if (el.fMonitorArcOpening) {
      el.fMonitorArcOpening.value = "left";
    }
    if (el.fMonitorArcOpeningWrap) {
      el.fMonitorArcOpeningWrap.classList.add("hidden");
    }
    if (el.fMonitorMin) {
      el.fMonitorMin.value = "";
    }
    if (el.fMonitorMax) {
      el.fMonitorMax.value = "";
    }
    if (el.fMonitorSubEntities) {
      el.fMonitorSubEntities.value = "";
    }
    if (el.tempOptions) {
      el.tempOptions.classList.add("hidden");
    }
    if (el.fTempStyleVariant) {
      el.fTempStyleVariant.value = "graphic";
    }
    if (el.fTempMin) {
      el.fTempMin.value = "";
    }
    ...
    if (el.entityListOptions) {
      el.entityListOptions.classList.add("hidden");
    }
    if (el.fEntityListIds) {
      el.fEntityListIds.value = "";
    }
    if (el.sensorTileOptions) {
      el.sensorTileOptions.classList.add("hidden");
    }
    if (el.fSensorTileIds) {
      el.fSensorTileIds.value = "";
    }
    if (el.fSensorTileLan) {
      el.fSensorTileLan.value = "";
    }
    if (el.fSensorTileWan) {
      el.fSensorTileWan.value = "";
    }
    if (el.fSensorTileTcp) {
      el.fSensorTileTcp.value = "";
    }
    if (el.fSensorTileUdp) {
      el.fSensorTileUdp.value = "";
    }
    if (el.fSensorTilePower) {
      el.fSensorTilePower.value = "";
    }
    if (el.binaryOptions) {
  ...
  if (el.fCardBgColor) {
    el.fCardBgColor.value = normalizeHexColor(widget.card_bg_color, "");
  }
  if (el.fLabelColor) {
    el.fLabelColor.value = normalizeHexColor(widget.label_color, "");
  }
  if (el.fBgImage) {
    el.fBgImage.value = widget.bg_image || "";
  }
  if (el.fBgImageFile) {
    el.fBgImageFile.value = "";
  }
  ...
  const isMonitor = widget.type === "monitor_tile";
  const isEntityList = widget.type === "entity_list";
  const isSensorTile = widget.type === "sensor_tile";
  const isTemp = widget.type === "temp_tile";
  if (el.buttonOptions) {
  ...
  if (el.monitorOptions) {
    el.monitorOptions.classList.toggle("hidden", !isMonitor);
  }
  if (el.entityListOptions) {
    el.entityListOptions.classList.toggle("hidden", !isEntityList);
  }
  if (el.sensorTileOptions) {
    el.sensorTileOptions.classList.toggle("hidden", !isSensorTile);
  }
  if (el.tempOptions) {
    el.tempOptions.classList.toggle("hidden", !isTemp);
  }
  if (el.binaryOptions) {
  ...
  if (isSensor) {
    const styleVariant = SENSOR_STYLE_VARIANTS.has(widget.style_variant) ? widget.style_variant : "default";
    const arcOpening = ARC_OPENINGS.has(widget.arc_opening) ? widget.arc_opening : "left";
    widget.style_variant = styleVariant;
    widget.arc_opening = arcOpening;
    if (el.fSensorStyleVariant) {
      el.fSensorStyleVariant.value = styleVariant;
    }
    if (el.fSensorArcOpening) {
      el.fSensorArcOpening.value = arcOpening;
    }
    if (el.fSensorArcOpeningWrap) {
      el.fSensorArcOpeningWrap.classList.toggle("hidden", styleVariant !== "arc" && styleVariant !== "arc_semi");
    }
    if (el.fSensorMin) {
      el.fSensorMin.value = typeof widget.sensor_min === "number" ? String(widget.sensor_min) : "";
    }
    if (el.fSensorMax) {
      el.fSensorMax.value = typeof widget.sensor_max === "number" ? String(widget.sensor_max) : "";
    }
  } else {
    if (el.fSensorStyleVariant) {
      el.fSensorStyleVariant.value = "default";
  ...
  if (isMonitor) {
    const styleVariant = MONITOR_STYLE_VARIANTS.has(widget.style_variant) ? widget.style_variant : "default";
    const arcOpening = ARC_OPENINGS.has(widget.arc_opening) ? widget.arc_opening : "left";
    widget.style_variant = styleVariant;
    widget.arc_opening = arcOpening;
    if (el.fMonitorStyleVariant) {
      el.fMonitorStyleVariant.value = styleVariant;
    }
    if (el.fMonitorArcOpening) {
      el.fMonitorArcOpening.value = arcOpening;
    }
    if (el.fMonitorArcOpeningWrap) {
      el.fMonitorArcOpeningWrap.classList.toggle("hidden", !MONITOR_ARC_STYLES.has(styleVariant));
    }
    if (el.fMonitorMin) {
      el.fMonitorMin.value = typeof widget.sensor_min === "number" ? String(widget.sensor_min) : "";
    }
    if (el.fMonitorMax) {
      el.fMonitorMax.value = typeof widget.sensor_max === "number" ? String(widget.sensor_max) : "";
    }
    if (el.fMonitorSubEntities) {
      el.fMonitorSubEntities.value = widget.sub_entity_ids || "";
    }
  } else {
    if (el.fMonitorStyleVariant) {
      el.fMonitorStyleVariant.value = "default";
  ...
  if (isTemp) {
    const styleVariant = TEMP_STYLE_VARIANTS.has(widget.style_variant) ? widget.style_variant : "graphic";
    widget.style_variant = styleVariant;
    if (el.fTempStyleVariant) {
      el.fTempStyleVariant.value = styleVariant;
    }
    if (el.fTempMin) {
      el.fTempMin.value = typeof widget.sensor_min === "number" ? String(widget.sensor_min) : "";
    }
    if (el.fTempMax) {
      el.fTempMax.value = typeof widget.sensor_max === "number" ? String(widget.sensor_max) : "";
    }
  } else {
    if (el.fTempStyleVariant) {
      el.fTempStyleVariant.value = "graphic";
  ...
  if (isEntityList) {
    if (el.fEntityListIds) {
      el.fEntityListIds.value = widget.entity_ids || "";
    }
  } else if (el.fEntityListIds) {
    el.fEntityListIds.value = "";
  }

  if (isSensorTile) {
    const buckets = sensorTileClassifyRows(normalizeSensorTileRows(widget.entity_ids || ""));
    if (el.fSensorTileIds) {
      el.fSensorTileIds.value = buckets.rest.join(", ");
    }
    if (el.fSensorTileLan) {
      el.fSensorTileLan.value = buckets.lan;
    }
    if (el.fSensorTileWan) {
      el.fSensorTileWan.value = buckets.wan;
    }
    if (el.fSensorTileTcp) {
      el.fSensorTileTcp.value = buckets.tcp;
    }
    if (el.fSensorTileUdp) {
      el.fSensorTileUdp.value = buckets.udp;
    }
    if (el.fSensorTilePower) {
      el.fSensorTilePower.value = buckets.power;
    }
    if (el.fSensorTileTitleFont) {
      el.fSensorTileTitleFont.value = widget.sensor_tile_title_font_px || 0;
    }
    if (el.fSensorTileRowFont) {
      el.fSensorTileRowFont.value = widget.sensor_tile_row_font_px || 0;
    }
    if (el.fSensorTileIpFont) {
      el.fSensorTileIpFont.value = widget.sensor_tile_ip_font_px || 0;
    }
    if (el.fSensorTilePowerFont) {
      el.fSensorTilePowerFont.value = widget.sensor_tile_power_font_px || 0;
    }
    if (el.fSensorTilePortsFont) {
      el.fSensorTilePortsFont.value = widget.sensor_tile_ports_font_px || 0;
    }
  } else if (el.fSensorTileIds) {
    el.fSensorTileIds.value = "";
  }
  renderSensorTileRowsEditor();
...
function addCamerasPage() {
  const pageId = uniqueId("cameras", editor.layout.pages);
  editor.layout.pages.push({
    id: pageId,
    type: CAMERAS_PAGE_TYPE,
    title: t("layout.pages.cameras_title"),
    widgets: [],
  });
  editor.selectedPageId = pageId;
  editor.selectedWidgetId = null;
...
function addWidget(type, options = {}) {
  ...
  if (!pageAcceptsWidgets(page)) {
    setStatus(t(isXiaozhiPage(page)
      ? "layout.status.xiaozhi_page_only"
      : isCamerasPage(page)
        ? "layout.status.cameras_page_only"
        : "layout.status.energy_page_only"), true);
    return null;
  }
  ...
  const defaultW = compact
    ? type === "weather_3day" ? 420
      ...
      : type === "select" ? 200
      : type === "monitor_tile" ? 220
      : type === "entity_list" ? 240
      : type === "sensor_tile" ? 240
      : type === "temp_tile" ? 160
      : 180
    : type === "weather_3day" ? 360
      ...
      : type === "select" ? 260
      : type === "monitor_tile" ? 300
      : type === "entity_list" ? 320
      : type === "sensor_tile" ? 320
      : type === "temp_tile" ? 200
      : 220;
  const defaultH = compact
    ? type === "weather_3day" ? 240
      ...
      : type === "select" ? 110
      : type === "monitor_tile" ? 160
      : type === "entity_list" ? 200
    ...
    : type === "weather_3day" ? 260
      ...
      : type === "select" ? 140
      : type === "monitor_tile" ? 220
      : type === "entity_list" ? 260
      : type === "sensor_tile" ? 260
      : type === "temp_tile" ? 200
      : 120;
  ...
  if (type === "monitor_tile") {
    widget.style_variant = "default";
    widget.sub_entity_ids = "";
  }
  if (type === "temp_tile") {
    widget.style_variant = "graphic";
  }
  if (type === "entity_list") {
    const entityIds = Array.isArray(options.entityIds)
      ? options.entityIds.filter((id) => typeof id === "string").map((id) => id.trim()).filter(Boolean)
      : [];
    widget.entity_ids = entityIds.join(", ");
  }
  if (type === "sensor_tile") {
    const presetRows = Array.isArray(options.sensorTileRows)
      ? options.sensorTileRows
          ...
          .map((row) => {
            ...
            if (!label) return "";
            const suffix = sensorTileStyleSuffix(row.style, row.powerColor);
            return `${label}=${suffix}`;
          })
          .filter((row) => row.length > 1)
      : [];
    widget.entity_ids = presetRows.join(", ");
  }
...
function sensorTileStripStyle(entity) {
  let style = "dot";
  let powerColor = "";
  let cleaned = String(entity || "").trim();
  /* ":power" may carry an optional colour: ":power", ":power:red",
   * ":power:#RRGGBB". Check it first so ":power:red" isn't misread. */
  const powerMatch = cleaned.match(/:power(?::([a-zA-Z]+|#[0-9a-fA-F]{6}))?$/);
  if (powerMatch) {
    style = "power";
    powerColor = powerMatch[1] || "";
    cleaned = cleaned.slice(0, powerMatch.index).trim();
  } else if (cleaned.endsWith(":bar")) {
    style = "bar";
    cleaned = cleaned.slice(0, -4).trim();
  } else if (cleaned.endsWith(":dot")) {
    style = "dot";
    cleaned = cleaned.slice(0, -4).trim();
  } else if (cleaned.endsWith(":status")) {
    style = "status";
    cleaned = cleaned.slice(0, -7).trim();
  } else if (cleaned.endsWith(":chart")) {
    style = "chart";
    cleaned = cleaned.slice(0, -6).trim();
  } else if (cleaned.endsWith(":ports")) {
    style = "ports";
    cleaned = cleaned.slice(0, -6).trim();
  } else if (cleaned.endsWith(":ip")) {
    style = "ip";
    cleaned = cleaned.slice(0, -3).trim();
  }
  return { entity: cleaned, style, powerColor };
}

function sensorTileStyleSuffix(style, powerColor) {
  if (style === "bar") return ":bar";
  if (style === "status") return ":status";
  if (style === "chart") return ":chart";
  if (style === "ports") return ":ports";
  if (style === "ip") return ":ip";
  if (style === "power") {
    const color = String(powerColor || "").trim();
    return color ? `:power:${color}` : ":power";
  }
  return "";
}

function normalizeSensorTileRows(value) {
  const rows = [];
  for (const segment of String(value || "").split(/[\n,]+/)) {
    const trimmed = segment.trim();
    if (!trimmed) continue;
    const eq = trimmed.indexOf("=");
    const label = eq >= 0 ? trimmed.slice(0, eq).trim() : trimmed;
    const rawEntity = eq >= 0 ? trimmed.slice(eq + 1).trim() : "";
    if (!label) continue;
    const { entity, style, powerColor } = sensorTileStripStyle(rawEntity);
    const suffix = sensorTileStyleSuffix(style, powerColor);
    rows.push(`${label}=${entity}${suffix}`);
  }
  return rows.slice(0, 24);
}

function sensorTileClassifyRows(rows) {
  const buckets = { lan: "", wan: "", tcp: "", udp: "", power: "", rest: [] };
  let genericIpIndex = 0;
  for (const row of rows) {
    const eq = row.indexOf("=");
    const label = eq >= 0 ? row.slice(0, eq).trim() : "";
    const rawEntity = eq >= 0 ? row.slice(eq + 1).trim() : "";
    if (!label) continue;
    const parsed = sensorTileStripStyle(rawEntity);
    const entity = parsed.entity;
    if (!entity) {
      /* Keep rows whose entity is empty (e.g. "GPU=:bar") so they survive
       * editor round-trips; an empty entity simply hides the row. */
      buckets.rest.push(row);
      continue;
    }
    const lc = label.toLowerCase();
    if (parsed.style === "power" ||
        (parsed.style === "dot" && ["moc", "power", "wat", "watts"].includes(lc))) {
      buckets.power = buckets.power || entity;
    } else if (parsed.style === "ports" || (parsed.style === "dot" && ["tcp", "udp"].includes(lc))) {
      if (lc === "udp") {
        buckets.udp = buckets.udp || entity;
      } else {
        buckets.tcp = buckets.tcp || entity;
      }
    } else if (parsed.style === "ip" || (parsed.style === "dot" && ["lan", "wan"].includes(lc))) {
      if (lc === "wan") {
        buckets.wan = buckets.wan || entity;
      } else if (lc === "lan") {
        buckets.lan = buckets.lan || entity;
      } else if (genericIpIndex === 0) {
        buckets.lan = buckets.lan || entity;
        genericIpIndex++;
      } else {
        buckets.wan = buckets.wan || entity;
...
function sensorTileCombineRows(buckets) {
  const rows = [];
  const clean = (value) => {
    const trimmed = String(value || "").trim();
    if (!trimmed) return "";
    return sensorTileStripStyle(trimmed).entity;
  };
  const lan = clean(buckets.lan);
  const wan = clean(buckets.wan);
  const tcp = clean(buckets.tcp);
  const udp = clean(buckets.udp);
  const power = clean(buckets.power);
  if (lan) rows.push(`LAN=${lan}:ip`);
  if (wan) rows.push(`WAN=${wan}:ip`);
  if (tcp) rows.push(`TCP=${tcp}:ports`);
  if (udp) rows.push(`UDP=${udp}:ports`);
  if (power) rows.push(`Moc=${power}:power`);
  for (const row of buckets.rest) {
    rows.push(row);
  }
  return rows.slice(0, 24);
}

function sensorTileRowEditorSync() {
  if (!el.fSensorTileIds || !el.sensorTileRowsEditor) return;
  const rows = [];
  const nodes = el.sensorTileRowsEditor.querySelectorAll(".st-row");
  for (const node of nodes) {
    const labelInput = node.querySelector(".st-label");
    const entityInput = node.querySelector(".st-entity");
    const label = (labelInput ? labelInput.value : "").trim();
    const { entity } = sensorTileStripStyle(entityInput ? entityInput.value : "");
    if (!label && !entity) continue;
    const suffix = sensorTileStyleSuffix(node.dataset.style, node.dataset.powerColor);
    rows.push(`${label}=${entity}${suffix}`);
  }
  el.fSensorTileIds.value = rows.join(", ");
}

function sensorTileRowEditorAppendRow(label, entity, style, powerColor, focus) {
  const container = el.sensorTileRowsEditor;
  ...
  rowEl.dataset.style =
    ...
    style === "ports" ? "ports" :
    style === "ip" ? "ip" :
    style === "power" ? "power" : "dot";
  rowEl.dataset.powerColor = String(powerColor || "").trim();

  let colorInput = null;

  const styleBtn = document.createElement("button");
  styleBtn.type = "button";
  styleBtn.className = "st-style";
  styleBtn.title = "Toggle dot / bar / chart / status / ports / IP / power";
  const updateStyleBtn = () => {
    const current = rowEl.dataset.style;
    if (current === "bar") {
      styleBtn.textContent = "▮";
      styleBtn.title = "Bar — click for chart";
    } else if (current === "chart") {
      styleBtn.textContent = "∿";
      styleBtn.title = "Chart (sparkline) — click for status";
    } else if (current === "status") {
      styleBtn.textContent = "◉";
      styleBtn.title = "Status — click for ports";
    } else if (current === "ports") {
      styleBtn.textContent = "⇄";
      styleBtn.title = "Ports (wrapped list) — click for IP";
    } else if (current === "ip") {
      styleBtn.textContent = "IP";
      styleBtn.title = "IP address — click for power";
    } else if (current === "power") {
      styleBtn.textContent = "⚡";
      styleBtn.title = "Power (watts + sparkline) — click for dot";
    } else {
      styleBtn.textContent = "●";
      styleBtn.title = "Dot — click for bar";
    }
    styleBtn.classList.toggle("is-bar", current === "bar");
    styleBtn.classList.toggle("is-chart", current === "chart");
    styleBtn.classList.toggle("is-status", current === "status");
    styleBtn.classList.toggle("is-ports", current === "ports");
    styleBtn.classList.toggle("is-ip", current === "ip");
    styleBtn.classList.toggle("is-power", current === "power");
    if (colorInput) {
      colorInput.style.display = current === "power" ? "" : "none";
    }
  };
  styleBtn.onclick = () => {
    const current = rowEl.dataset.style;
    rowEl.dataset.style =
      ...
      current === "ports" ? "ip" :
      current === "ip" ? "power" :
      current === "power" ? "dot" : "bar";
    updateStyleBtn();
    sensorTileRowEditorSync();
    autoApplyInspector({ refreshInspector: false, softEntityValidation: true });
  };
  updateStyleBtn();

  const labelInput = document.createElement("input");
  labelInput.type = "text";
  labelInput.className = "st-label";
  labelInput.placeholder = "Label";
  labelInput.value = label;

  const entityInput = document.createElement("input");
  entityInput.type = "text";
  entityInput.className = "st-entity";
  entityInput.setAttribute("list", "entityOptions");
  entityInput.placeholder = "entity.id (empty hides row)";
  entityInput.value = entity;

  colorInput = document.createElement("input");
  colorInput.type = "color";
  colorInput.className = "st-power-color";
  colorInput.title = "Power colour";
  colorInput.value = /^#[0-9a-fA-F]{6}$/.test(rowEl.dataset.powerColor)
    ? rowEl.dataset.powerColor
    : "#ff5252";

  const removeBtn = document.createElement("button");
  removeBtn.type = "button";
  removeBtn.className = "st-remove";
  ...
  labelInput.addEventListener("input", syncInput);
  entityInput.addEventListener("input", syncInput);
  entityInput.addEventListener("change", () => {
    sensorTileRowEditorSync();
    autoApplyInspector({ refreshInspector: true, softEntityValidation: true });
  });
  colorInput.addEventListener("input", () => {
    rowEl.dataset.powerColor = colorInput.value;
    sensorTileRowEditorSync();
    autoApplyInspector({ refreshInspector: false, softEntityValidation: true });
  });

  rowEl.appendChild(styleBtn);
  rowEl.appendChild(labelInput);
  rowEl.appendChild(entityInput);
...
function renderSensorTileRowsEditor() {
  const container = el.sensorTileRowsEditor;
  if (!container || !el.fSensorTileIds) return;
  container.innerHTML = "";

  const rows = normalizeSensorTileRows(el.fSensorTileIds.value);
  for (const row of rows) {
    const eq = row.indexOf("=");
    const label = eq >= 0 ? row.slice(0, eq) : row;
    const rawEntity = eq >= 0 ? row.slice(eq + 1) : "";
    const { entity, style, powerColor } = sensorTileStripStyle(rawEntity);
    sensorTileRowEditorAppendRow(label, entity, style, powerColor, false);
  }
  if (rows.length === 0) {
    sensorTileRowEditorAppendRow("", "", "dot", true);
  }
}
...
function applyInspector(options = {}) {
  ...
  const isEntityList = widgetType === "entity_list";
  const isSensorTile = widgetType === "sensor_tile";
  const listLike = isEntityList || isSensorTile;
  const nextEntityId = listLike
    ? ""
    : el.fEntity.value.trim() || pickDefaultEntityForWidgetType(widgetType, sliderDomain, buttonMode);

  const primaryEntityValid = listLike
  ...
  {
    const cardBgColor = normalizeHexColor(el.fCardBgColor?.value, "");
    if (cardBgColor) {
      widget.card_bg_color = cardBgColor;
    } else {
      delete widget.card_bg_color;
    }
    const labelColor = normalizeHexColor(el.fLabelColor?.value, "");
    if (labelColor) {
      widget.label_color = labelColor;
    } else {
      delete widget.label_color;
    }
    const bgImage = (el.fBgImage?.value || "").trim();
    if (bgImage) {
      widget.bg_image = bgImage;
    } else {
      delete widget.bg_image;
    }
  }
  if (listLike) {
    delete widget.entity_id;
  ...
  if (widgetType === "heating_tile") {
    ...
    delete widget.arc_opening;
  } else if (widgetType === "sensor") {
    const variant = SENSOR_STYLE_VARIANTS.has(el.fSensorStyleVariant?.value)
      ? el.fSensorStyleVariant.value
      : "default";
    widget.style_variant = variant;
    if (variant === "arc" || variant === "arc_semi") {
      const opening = el.fSensorArcOpening?.value;
      widget.arc_opening = ARC_OPENINGS.has(opening) ? opening : "left";
    } else {
      delete widget.arc_opening;
    }
    const sensorMin = normalizeOptionalNumber(el.fSensorMin?.value);
    const sensorMax = normalizeOptionalNumber(el.fSensorMax?.value);
    if (sensorMin !== null) {
      widget.sensor_min = sensorMin;
    } else {
      delete widget.sensor_min;
    }
    if (sensorMax !== null) {
      widget.sensor_max = sensorMax;
    } else {
      delete widget.sensor_max;
    }
  } else if (widgetType === "monitor_tile") {
    const variant = MONITOR_STYLE_VARIANTS.has(el.fMonitorStyleVariant?.value)
      ? el.fMonitorStyleVariant.value
      : "default";
    widget.style_variant = variant;
    if (MONITOR_ARC_STYLES.has(variant)) {
      const opening = el.fMonitorArcOpening?.value;
      widget.arc_opening = ARC_OPENINGS.has(opening) ? opening : "left";
    } else {
      delete widget.arc_opening;
    }
    const sensorMin = normalizeOptionalNumber(el.fMonitorMin?.value);
    const sensorMax = normalizeOptionalNumber(el.fMonitorMax?.value);
    if (sensorMin !== null) {
      widget.sensor_min = sensorMin;
    } else {
      delete widget.sensor_min;
    ...
    const subEntities = normalizeEntityIdList(el.fMonitorSubEntities?.value).slice(0, 4);
    widget.sub_entity_ids = subEntities.join(", ");
  } else if (widgetType === "temp_tile") {
    const variant = TEMP_STYLE_VARIANTS.has(el.fTempStyleVariant?.value)
      ? el.fTempStyleVariant.value
      : "graphic";
    widget.style_variant = variant;
    delete widget.arc_opening;
    const sensorMin = normalizeOptionalNumber(el.fTempMin?.value);
    const sensorMax = normalizeOptionalNumber(el.fTempMax?.value);
    if (sensorMin !== null) {
      widget.sensor_min = sensorMin;
    } else {
      delete widget.sensor_min;
    ...
    const entityIds = normalizeEntityIdList(el.fEntityListIds?.value).slice(0, 24);
    widget.entity_ids = entityIds.join(", ");
    delete widget.style_variant;
    delete widget.arc_opening;
  } else if (isSensorTile) {
    const buckets = {
      lan: el.fSensorTileLan?.value || "",
      wan: el.fSensorTileWan?.value || "",
      tcp: el.fSensorTileTcp?.value || "",
      udp: el.fSensorTileUdp?.value || "",
      power: el.fSensorTilePower?.value || "",
      rest: normalizeSensorTileRows(el.fSensorTileIds?.value),
    };
    widget.entity_ids = sensorTileCombineRows(buckets).join(", ");
    delete widget.style_variant;
    delete widget.arc_opening;
    const titleFont = normalizeOptionalNumber(el.fSensorTileTitleFont?.value);
    const rowFont = normalizeOptionalNumber(el.fSensorTileRowFont?.value);
    const ipFont = normalizeOptionalNumber(el.fSensorTileIpFont?.value);
    const powerFont = normalizeOptionalNumber(el.fSensorTilePowerFont?.value);
    const portsFont = normalizeOptionalNumber(el.fSensorTilePortsFont?.value);
    if (titleFont !== null && titleFont > 0) {
      widget.sensor_tile_title_font_px = titleFont;
    } else {
      delete widget.sensor_tile_title_font_px;
    }
    if (rowFont !== null && rowFont > 0) {
      widget.sensor_tile_row_font_px = rowFont;
    } else {
      delete widget.sensor_tile_row_font_px;
    }
    if (ipFont !== null && ipFont > 0) {
      widget.sensor_tile_ip_font_px = ipFont;
    } else {
      delete widget.sensor_tile_ip_font_px;
    }
    if (powerFont !== null && powerFont > 0) {
      widget.sensor_tile_power_font_px = powerFont;
    } else {
      delete widget.sensor_tile_power_font_px;
    }
    if (portsFont !== null && portsFont > 0) {
      widget.sensor_tile_ports_font_px = portsFont;
    } else {
      delete widget.sensor_tile_ports_font_px;
    }
  } else {
    delete widget.style_variant;
    delete widget.arc_opening;
  ...
  if (widgetType !== "monitor_tile") {
    delete widget.sub_entity_ids;
  }
  if (widgetType !== "sensor" && widgetType !== "monitor_tile" && widgetType !== "temp_tile") {
    delete widget.sensor_min;
    delete widget.sensor_max;
  }
...
function bindUi() {
  ...
  if (el.addCamerasPageBtn) {
    el.addCamerasPageBtn.onclick = addCamerasPage;
  }
  ...
  if (el.addMonitorTileBtn) {
    el.addMonitorTileBtn.onclick = () => openLightEntityPicker("monitor_tile");
  }
  if (el.addEntityListBtn) {
    el.addEntityListBtn.onclick = () => openLightEntityPicker("entity_list");
  }
  if (el.addSensorTileBtn) {
    el.addSensorTileBtn.onclick = () => addWidget("sensor_tile");
  }
  if (el.addTemperatureTileBtn) {
    el.addTemperatureTileBtn.onclick = () => addWidget("temp_tile", {
      title: "CPU",
    });
  }
  if (el.lightEntityPickerRefreshBtn) {
  ...
  if (el.lightEntityPickerConfirmBtn) {
    el.lightEntityPickerConfirmBtn.onclick = () => {
      const config = entityPickerConfig();
      const widgetType = config.widgetType || editor.lightPicker.widgetType;
      const ids = editor.lightPicker.selected;
      if (!ids.length) return;
      const widgetLabel = t(config.widgetKey, {}, config.widgetFallback);
      addWidget(widgetType, { entityIds: ids });
      closeLightEntityPicker();
      setStatus(t("entity_picker.added_widget", { widget: widgetLabel, entity: ids.join(", ") }));
    };
  }
  if (el.lightEntityPickerOverlay) {
  ...
  if (el.setupWizardAddBinaryBtn) {
    el.setupWizardAddBinaryBtn.onclick = () => openSetupWizardEntityPicker("binary_sensor");
  }
  if (el.setupWizardAddMonitorBtn) {
    el.setupWizardAddMonitorBtn.onclick = () => openSetupWizardEntityPicker("monitor_tile");
  }
  if (el.setupWizardAddEntityListBtn) {
    el.setupWizardAddEntityListBtn.onclick = () => openSetupWizardEntityPicker("entity_list");
  }
  if (el.setupWizardAddSensorTileBtn) {
    el.setupWizardAddSensorTileBtn.onclick = () => addWidget("sensor_tile");
  }
  if (el.setupWizardAddTemperatureTileBtn) {
    el.setupWizardAddTemperatureTileBtn.onclick = () => addWidget("temp_tile", {
      title: "CPU",
    });
  }
  if (el.setupWizardAddMachineBtn) {
    el.setupWizardAddMachineBtn.onclick = () => addWidget("sensor_tile", {
      title: "Maszyna / CPU",
      sensorTileRows: [
        { label: "CPU", style: "bar" },
        { label: "RAM", style: "bar" },
        { label: "GPU", style: "bar" },
        { label: "Temp", style: "dot" },
      ],
    });
  }
  if (el.setupWizardAddServerBtn) {
    el.setupWizardAddServerBtn.onclick = () => addWidget("sensor_tile", {
      title: "Serwery",
      sensorTileRows: [
        { label: "Status", style: "status" },
        { label: "Uptime", style: "dot" },
        { label: "Dysk", style: "bar" },
        { label: "Load", style: "bar" },
      ],
    });
  }
  if (el.setupWizardAddNetworkBtn) {
    el.setupWizardAddNetworkBtn.onclick = () => addWidget("sensor_tile", {
      title: "Sieć",
      sensorTileRows: [
        { label: "LAN ↓", style: "chart" },
        { label: "LAN ↑", style: "chart" },
        { label: "WAN ↓", style: "chart" },
        { label: "WAN ↑", style: "chart" },
      ],
    });
  }
  if (el.setupWizardAddGlancesBtn) {
    el.setupWizardAddGlancesBtn.onclick = () => addWidget("sensor_tile", {
      title: "Glances",
      sensorTileRows: [
        { label: "CPU", style: "bar" },
        { label: "RAM", style: "bar" },
        { label: "Temp", style: "dot" },
        { label: "Sieć", style: "dot" },
      ],
    });
  }
  ...
  if (el.fBgImageUploadBtn) {
    el.fBgImageUploadBtn.onclick = () => uploadTileBackground();
  }
  if (el.fBgImageRefreshBtn) {
    el.fBgImageRefreshBtn.onclick = () => loadTileBackgroundList();
  }
  if (el.fBgImageSelect) {
    el.fBgImageSelect.onchange = () => {
      if (el.fBgImage) {
        el.fBgImage.value = el.fBgImageSelect.value;
      }
      autoApplyInspector({ softEntityValidation: true });
    };
  }
  loadTileBackgroundList();
  if (el.sdListRefreshBtn) {
    el.sdListRefreshBtn.onclick = () => loadSdFileList();
  }
  loadSdFileList();
  ...
  el.fType.onchange = () => {
    ...
    if (el.monitorOptions) {
      el.monitorOptions.classList.toggle("hidden", el.fType.value !== "monitor_tile");
    }
    if (el.tempOptions) {
      el.tempOptions.classList.toggle("hidden", el.fType.value !== "temp_tile");
    }
    if (el.entityListOptions) {
      el.entityListOptions.classList.toggle("hidden", el.fType.value !== "entity_list");
    }
    if (el.sensorTileOptions) {
      el.sensorTileOptions.classList.toggle("hidden", el.fType.value !== "sensor_tile");
    }
    updateSensorArcOpeningWrapVisibility();
    renderEntityOptions();
  ...
  bindInspectorAutoApply(el.fCardBgColor, ["change"], { softEntityValidation: true });
  bindInspectorAutoApply(el.fLabelColor, ["change"], { softEntityValidation: true });
  bindInspectorAutoApply(el.fBgImage, ["change"], { softEntityValidation: true });
  ...
  bindInspectorAutoApply(el.fSensorStyleVariant, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fSensorArcOpening, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fSensorMin, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fSensorMax, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fMonitorStyleVariant, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fMonitorArcOpening, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fMonitorMin, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fMonitorMax, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fMonitorSubEntities, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fTempStyleVariant, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fTempMin, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fTempMax, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fEntityListIds, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fSensorTileIds, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fSensorTileTitleFont, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fSensorTileRowFont, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fSensorTileIpFont, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fSensorTilePowerFont, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fSensorTilePortsFont, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fSensorTileLan, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fSensorTileWan, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fSensorTileTcp, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fSensorTileUdp, ["change"], { refreshInspector: true, softEntityValidation: true });
  bindInspectorAutoApply(el.fSensorTilePower, ["change"], { refreshInspector: true, softEntityValidation: true });
  if (el.sensorTileAddRowBtn) {
    el.sensorTileAddRowBtn.onclick = () => {
      sensorTileRowEditorSync();
      sensorTileRowEditorAppendRow("", "", "dot", true);
    };
  }
  ...
  if (el.settingsScreensaverWallpaperUploadBtn) {
    el.settingsScreensaverWallpaperUploadBtn.onclick = async () => {
      ...
      try {
        ...
        if (el.settingsScreensaverWallpaperInfo) {
          ...
          el.settingsScreensaverWallpaperInfo.classList.add("error");
        }
      }
    };
  }
  ...
  if (el.refreshSdStatusBtn) {
    el.refreshSdStatusBtn.onclick = () => {
      void loadSdStatus();
    };
  }
  if (el.formatSdBtn) {
    el.formatSdBtn.onclick = () => {
      void formatSdCard();
    };
  }
  if (el.backupConfigBtn) {
    el.backupConfigBtn.onclick = () => {
      void backupConfig();
    };
  }
  if (el.restoreConfigBtn) {
    el.restoreConfigBtn.onclick = () => {
      void restoreConfig();
    };
  }
  if (el.restartDeviceBtn) {
    el.restartDeviceBtn.onclick = () => {
      void restartDevice();
    };
  }
...
function initSimpleUiMenus() {
  ...
  const pageCameras = document.getElementById("addPageMenuCameras");
  if (pageCameras) {
    pageCameras.onclick = () => {
      closeAllDropdowns();
      if (el.addCamerasPageBtn) el.addCamerasPageBtn.click();
    };
  }
