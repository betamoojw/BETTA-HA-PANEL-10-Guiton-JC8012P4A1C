...
void app_main(void)
{
    ...
    system_log_set_daily_restart_hour(s_runtime_settings.daily_restart_hour);
#if CONFIG_APP_PANEL_VARIANT_10INCH_JC
    data_log_set_enabled(s_runtime_settings.sd_logging_enabled);
    data_log_set_flush_interval_s(s_runtime_settings.sd_flush_interval_s);
    data_log_set_log_system_enabled(s_runtime_settings.sd_log_system_enabled);
    ...
    if (has_wifi_credentials) {
        wifi_mgr_config_t wifi_cfg = {
            ...
            .static_ip_enabled = s_runtime_settings.wifi_static_ip_enabled,
...
#if CONFIG_APP_PANEL_VARIANT_10INCH_JC
        /* The ES8311 codec shares the touch I2C bus, so init it after the
         * bus exists.  Best effort: absence of the codec must not block boot. */
        esp_err_t audio_err = audio_init();
        if (audio_err != ESP_OK) {
            ESP_LOGW(TAG_AUDIO, "Audio init failed, continuing without audio: %s", esp_err_to_name(audio_err));
        }
#if CONFIG_APP_FEATURE_XIAOZHI
        /* Apply the configured speaker volume once the codec is live. */
        (void)xz_audio_set_volume(s_runtime_settings.audio_volume);
#endif
#endif
...
#if CONFIG_APP_PANEL_VARIANT_10INCH_JC
        esp_err_t ss_err = ui_screensaver_init();
        if (ss_err != ESP_OK) {
            ESP_LOGW(TAG_UI, "Screensaver init failed: %s", esp_err_to_name(ss_err));
        }
#endif
