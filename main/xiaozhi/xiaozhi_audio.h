...
/* JC panel: a single ES8311 mono mic on the codec ADC (DSIN=GPIO11). */
#define XZ_MIC_CHANNELS 1
#else
...
// Get the maximum allowed volume (clean ceiling, see xiaozhi_audio.c)
...
// Set microphone gain in dB (ES8311 mono or ES7210 stereo), e.g. 24.0f.
esp_err_t xz_audio_set_mic_gain(float db);

// Play 16-bit mono PCM samples through the speaker. The buffer is
// processed in place (TTS de-esser low-pass) and must be writable.
esp_err_t xz_audio_play(int16_t *pcm, int sample_count);

// Read up to max_frames 16-bit frames from the mic. Returns the number of
// frames read, or a negative value on error. On success the buffer holds
// frames_read * XZ_MIC_CHANNELS samples (mono on the JC panel, interleaved
// L/R stereo on the 7B); the caller downmixes to mono before Opus encoding.
...
// Mic capture uses XZ_MIC_CHANNELS channels (mono on JC, stereo on 7B);
// speaker output is always mono.
