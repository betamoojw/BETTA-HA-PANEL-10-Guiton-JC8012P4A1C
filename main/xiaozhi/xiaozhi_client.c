...
#include "freertos/idf_additions.h"   /* xTaskCreateWithCaps / xStreamBufferCreateWithCaps */
...
static int16_t  s_pcm_cap_s[CAP_FRAME_SAMPLES * XZ_MIC_CHANNELS]; /* Raw capture frame */
...
static void ws_event_handler(void *arg, esp_event_base_t base, int32_t event_id, void *data)
{
    ...
    case WEBSOCKET_EVENT_CONNECTED:
        ...
        xz_ui_clear_chat();
        xz_ui_add_chat_message("system", "Połączono z serwerem Xiaozhi.");
        xz_ui_set_status("Łączenie z serwerem...");
        send_hello(ws);
        break;
    ...
    if (strcmp(type->valuestring, "hello") == 0) {
        ...
        if (cJSON_IsString(j_state)) {
            if (strcmp(j_state->valuestring, "start") == 0 ||
                ...
                xz_ui_set_status(s.listening ? "Słuchanie..." : "Gotowy");
            }
        }
...
/* Digital headroom                                                    */
/* ------------------------------------------------------------------ */
/* Xiaozhi TTS decodes pinned at 0.0 dBFS: the server normalises it so
 * hot that intersample peaks exceed full scale and the int16 conversion
 * hard-clips them (flat-tops on the loud, bass-heavy portions = the
 * "crackle on bass" symptom). The ES8311's fixed-gain 5 V analogue PA
 * also clips several dB below 0 dBFS, so TTS must be attenuated anyway.
 * -10 dB is applied via OPUS_SET_GAIN inside the decoder (see
 * handle_binary_frame), i.e. BEFORE the int16 saturation, which is what
 * preserves the intersample overs instead of clipping them. It matches
 * the ~ -10.3 dBFS sine level that was proven clean through the ES8311 +
 * NS4150B path, so it is the loudest setting that keeps decoder headroom;
 * the chirp diagnostics in xz_audio_test_beep() are the decisive
 * analogue-vs-Opus test for whatever crackle remains. */
...
static void handle_binary_frame(size_t frame_len)
{
     ...
     * has fully stopped, so no lock is needed here. Decode at the server's
     * native output rate (24 kHz) so sibilants (5-8 kHz) survive Opus
     * decode, then bandlimited-resample to the 16 kHz pipeline. -10 dB is
     * still applied INSIDE the decoder (Q8: -10 * 256 = -2560), BEFORE the
     * int16 saturation, so the int16 conversion never hard-clips the hot
     * 0 dBFS TTS into flat-tops, and the analogue PA stays at the level
     * proven clean (-10.3 dBFS sweep). */
    if (s.decoder == NULL) {
        int err = 0;
        ...
        ESP_LOGI(TAG, "decoder ready @ %d Hz (server rate %d Hz)", rate, s.server_sample_rate);

        int gerr = opus_decoder_ctl(s.decoder, OPUS_SET_GAIN(-2560));
        if (gerr != OPUS_OK) {
            ESP_LOGW(TAG, "OPUS_SET_GAIN failed: %d", gerr);
        }
    }
...
static void playback_task_fn(void *arg)
{
    (void)arg;
    for (;;) {
        /* Block until TTS data arrives. The I2S DMA ring (8 x 1024 frames =
         * ~512 ms @ 16 kHz, see audio_init_panel10jc.c) absorbs producer
         * jitter, so a briefly empty stream buffer is harmless and must not
         * be logged as an underrun (the 60 ms TTS chunk cadence made the old
         * 50 ms timeout log a false-positive "DMA starved" every cycle). */
...
static void capture_task_exit(void)
{
    ...
    /* Self-delete must match the PSRAM-stack creation in ensure_capture_task. */
...
static void capture_task_fn(void *arg)
{
    ...
    while (s.listening && s.ws_connected) {
        ...
        /* Downmix the captured frame to mono for Opus, tracking per-channel
         * levels.  The JC panel has one ES8311 mic (XZ_MIC_CHANNELS=1); the
         * 7B has two ES7210 mics (XZ_MIC_CHANNELS=2) that are averaged. */
        int32_t peak[2] = {0, 0};
        int64_t sum[2] = {0, 0};
        for (int i = 0; i < CAP_FRAME_SAMPLES; i++) {
            int32_t acc = 0;
            for (int ch = 0; ch < XZ_MIC_CHANNELS; ch++) {
                int32_t v = s_pcm_cap_s[i * XZ_MIC_CHANNELS + ch];
                acc += v;
                int32_t av = v < 0 ? -v : v;
                if (av > peak[ch]) peak[ch] = av;
                sum[ch] += (int64_t)v * v;
            }
            s_pcm_cap[i] = (int16_t)(acc / XZ_MIC_CHANNELS);
        }

        /* Report levels every ~0.5 s so a dead mic slot is obvious. */
        if ((++diag % 8) == 0) {
            int rms = (int)sqrtf((float)sum[0] / CAP_FRAME_SAMPLES);
            if (XZ_MIC_CHANNELS == 1) {
                ESP_LOGI(TAG, "mic peak=%d rms=%d | cap_stack_hwm=%u",
                         (int)peak[0], rms,
                         (unsigned)uxTaskGetStackHighWaterMark(NULL));
            } else {
                int rms_r = (int)sqrtf((float)sum[1] / CAP_FRAME_SAMPLES);
                ESP_LOGI(TAG, "mic L peak=%d rms=%d | R peak=%d rms=%d | cap_stack_hwm=%u",
                         (int)peak[0], rms, (int)peak[1], rms_r,
                         (unsigned)uxTaskGetStackHighWaterMark(NULL));
            }
        }
...
static void connect_task_fn(void *arg)
{
    ...
    if (bits & BIT_SERVER_HELLO) {
        ...
        set_state(XIAOZHI_STATE_ERROR);
        xz_ui_set_status("Nie udało się połączyć");
        xSemaphoreTake(s.mutex, portMAX_DELAY);
        s.ws = NULL;
        xSemaphoreGive(s.mutex);
        xSemaphoreGive(s.session_sem);
...
esp_err_t xz_xiaozhi_init(const xiaozhi_config_t *cfg)
{
    ...
    /* Playback PCM stream buffer in PSRAM: 16 KB that would otherwise sit in
     * the scarce internal RAM (see the I2S DMA sizing note in
     * audio_init_panel10jc.c). */
    s.playback = xStreamBufferCreateWithCaps(PLAYBACK_BUF_SIZE, 1,
    ...
    /* Playback task stack in PSRAM (runs forever, so no WithCaps delete). */
    BaseType_t ok = xTaskCreateWithCaps(playback_task_fn, "xiaozhi_play",
                                        4096, NULL, 7, NULL,
                                        MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
    if (ok != pdPASS) {
...
static esp_err_t ensure_capture_task(void)
{
    ...
    if (s.capture_task == NULL) {
        /* 32 KB capture stack (Opus encode uses ~13 KB of alloca) in PSRAM:
         * keeps the largest internal-RAM block free for the I2S DMA ring. */
        BaseType_t ok = xTaskCreateWithCaps(capture_task_fn, "xiaozhi_cap",
                                            XZ_CAP_TASK_STACK, NULL, 6,
                                            &s.capture_task,
                                            MALLOC_CAP_SPIRAM | MALLOC_CAP_8BIT);
        if (ok != pdPASS) {
            xSemaphoreGive(s.mutex);
