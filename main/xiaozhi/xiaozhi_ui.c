    ...
    if (st.xiaozhi_enabled) {
        ...
        if (dev_id[0] != '\0') {
            snprintf(body, sizeof(body),
                     "Urządzenie: %s\n\nŁączenie z chmurą Xiaozhi...\nKod parowania pojawi się za chwilę.", dev_id);
        } else {
            strlcpy(body, "Łączenie z chmurą Xiaozhi...\nKod parowania pojawi się za chwilę.", sizeof(body));
        }
    } else {
        char url[96] = {0};
        ...
        if (dev_id[0] != '\0') {
            snprintf(body, sizeof(body),
                     "Asystent głosowy jest wyłączony.\n\nAby go włączyć:\n"
                     "1. Otwórz %s w przeglądarce\n"
                     "2. Przejdź do Ustawienia -> Xiaozhi AI\n"
                     "3. Zaznacz 'Włącz Xiaozhi' i zapisz\n\n"
                     "Urządzenie: %s",
                     url, dev_id);
        } else {
            snprintf(body, sizeof(body),
...
static void xz_ui_build_ptt_group(lv_obj_t *group)
{
    ...
    s_status_label = xz_make_label(group, "Dotknij, aby mówić",
...
static void xz_ui_show_setup_view(const char *title, const char *code, const char *body)
{
    ...
    /* The label width is set at build time to (APP_CONTENT_BOX_WIDTH - xz_x(160)),
     * so center it against that known width instead of lv_obj_get_width() which
     * returns 0 before layout and used to push the text off the right edge. */
    lv_coord_t body_w = APP_CONTENT_BOX_WIDTH - xz_x(160);
    lv_obj_set_pos(s_setup_body, (APP_CONTENT_BOX_WIDTH - body_w) / 2, body_y);
...
static void xz_ui_show_ptt_view(void)
{
    ...
    if (s_status_label != NULL) {
        lv_label_set_text(s_status_label,
                          xz_xiaozhi_configured() ? "Dotknij, aby mówić" : "Skonfiguruj Xiaozhi w panelu WWW");
    }
...
void xz_ui_set_listening(bool listening)
{
    ...
    display_lock(0);
    lv_label_set_text(s_status_label, listening ? "Słuchanie..." : "Dotknij, aby mówić");
    if (s_mic_btn) {
