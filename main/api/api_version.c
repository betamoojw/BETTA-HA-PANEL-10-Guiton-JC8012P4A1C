...
esp_err_t api_version_get_handler(httpd_req_t *req)
{
    ...
    /* Feature flags so the web editor can hide tabs whose routes are
     * compiled out of this build (e.g. Cameras on the JC panel). */
#if CONFIG_APP_FEATURE_CAMERAS
    cJSON_AddBoolToObject(root, "feature_cameras", true);
#else
    cJSON_AddBoolToObject(root, "feature_cameras", false);
#endif
    ...
    cJSON_AddBoolToObject(root, "feature_xiaozhi", true);
#else
    cJSON_AddBoolToObject(root, "feature_xiaozhi", false);
#endif
