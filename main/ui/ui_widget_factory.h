...
typedef struct {
    ...
    char card_bg_color[APP_MAX_COLOR_STR_LEN];
    char label_color[APP_MAX_COLOR_STR_LEN];
    char bg_image[APP_MAX_IMAGE_PATH_LEN];
    int x;
    int y;
    int w;
    int h;
...
typedef struct {
    ...
    char label_color[APP_MAX_COLOR_STR_LEN];
    char bg_image[APP_MAX_IMAGE_PATH_LEN];
    lv_image_dsc_t bg_image_dsc; /* decoded background, owned by the widget */
    bool visible;
    char extra_entity_ids[APP_MAX_EXTRA_ENTITY_IDS_LEN];
    void *ctx;
    lv_obj_t *obj;
