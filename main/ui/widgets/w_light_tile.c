...
/* TEMPORARY touch diagnostics: log the full press/drag event flow on the
 * light tile card + slider to pinpoint why slider drags fail while taps work. */
static const char *light_dbg_event_name(lv_event_code_t code)
{
    switch (code) {
...
static void light_dbg_event_cb(lv_event_t *event)
{
    lv_event_code_t code = lv_event_get_code(event);
    const char *name = light_dbg_event_name(code);
    if (name == NULL) {
        return;
    }

    lv_obj_t *target = lv_event_get_target(event);
    const bool is_slider = (lv_obj_get_class(target) == &lv_slider_class);
    ...
    lv_obj_t *scroll = (indev != NULL) ? lv_indev_get_scroll_obj(indev) : NULL;
    lv_area_t ca;
    lv_obj_get_coords(target, &ca);
    ESP_LOGW("DBG_TILE", "%s %s pt=(%d,%d) obj=(%d,%d,%d,%d) val=%d scroll=%p",
        is_slider ? "slider" : "card", name, (int)p.x, (int)p.y,
        (int)ca.x1, (int)ca.y1, (int)ca.x2, (int)ca.y2,
        is_slider ? (int)lv_slider_get_value(target) : -1, (void *)scroll);
}

/* Make the whole slider track a valid press target (not just the small knob).
 * LVGL v9 sliders reject presses anywhere except the knob by default, which made
 * track presses fall through to the card and toggle the light instead of moving
 * the slider. */
static void light_slider_hit_test_cb(lv_event_t *event)
{
    ...
    /* 1) The slider's own coords, generously expanded. */
    lv_area_t a;
    lv_obj_get_coords(slider, &a);
    lv_area_increase(&a, 0, 14);
    if (p->x >= a.x1 && p->x <= a.x2 && p->y >= a.y1 && p->y <= a.y2) {
        info->res = true;
        return;
    }

    /* 2) Fallback anchored to the card: the slider always sits at the bottom
     * of its card, so any press in the card's bottom band belongs to the
     * slider.  This stays correct even if the slider's own coords are stale
     * (e.g. the card was resized after the slider was last aligned), which
     * otherwise lets the press fall through to the card and toggle the light
     * instead of sliding. */
    if (card != NULL) {
        lv_area_t ca;
        lv_obj_get_coords(card, &ca);
        lv_coord_t card_h = ca.y2 - ca.y1 + 1;
        lv_coord_t band = lv_obj_get_height(slider) + 34;
        if (band < 54) {
            band = 54;
        }
        if (band > card_h) {
        ...
        ca.x1 -= 10;
        ca.x2 += 10;
        if (p->x >= ca.x1 && p->x <= ca.x2 && p->y >= ca.y1 && p->y <= ca.y2) {
            info->res = true;
        }
    }
}
...
esp_err_t w_light_tile_create(const ui_widget_def_t *def, lv_obj_t *parent, ui_widget_instance_t *out_instance)
{
    ...
    lv_obj_add_event_cb(slider, light_slider_hit_test_cb, LV_EVENT_HIT_TEST, NULL);
    ...
    lv_obj_add_event_cb(slider, light_dbg_event_cb, LV_EVENT_ALL, NULL);
    lv_obj_add_event_cb(card, light_dbg_event_cb, LV_EVENT_ALL, NULL);
