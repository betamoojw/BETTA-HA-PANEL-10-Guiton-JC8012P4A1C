...
typedef enum {
    W_SENSOR_STYLE_DEFAULT = 0,
    W_SENSOR_STYLE_PERCENT,
    W_SENSOR_STYLE_ARC,
    W_SENSOR_STYLE_ARC_SEMI,
} w_sensor_style_t;

typedef enum {
    W_SENSOR_OPENING_LEFT = 0,
    W_SENSOR_OPENING_RIGHT,
    W_SENSOR_OPENING_TOP,
    W_SENSOR_OPENING_BOTTOM,
} w_sensor_opening_t;
...
typedef struct {
    ...
    lv_obj_t *bar;
    lv_obj_t *arc;
    w_sensor_style_t style;
    w_sensor_opening_t opening;
    int min;
    int max;
...
static w_sensor_style_t sensor_style_from_variant(const char *variant)
{
    if (variant == NULL || variant[0] == '\0' || strcmp(variant, "default") == 0) {
        return W_SENSOR_STYLE_DEFAULT;
    }
    if (strcmp(variant, "percent") == 0 || strcmp(variant, "percent_bar") == 0) {
        return W_SENSOR_STYLE_PERCENT;
    }
    if (strcmp(variant, "arc") == 0) {
        return W_SENSOR_STYLE_ARC;
    }
    if (strcmp(variant, "arc_semi") == 0) {
        return W_SENSOR_STYLE_ARC_SEMI;
    }
    return W_SENSOR_STYLE_DEFAULT;
}

static w_sensor_opening_t sensor_opening_from_variant(const char *variant)
{
    if (variant != NULL) {
        if (strcmp(variant, "right") == 0) {
            return W_SENSOR_OPENING_RIGHT;
        }
        if (strcmp(variant, "top") == 0) {
            return W_SENSOR_OPENING_TOP;
        }
        if (strcmp(variant, "bottom") == 0) {
            return W_SENSOR_OPENING_BOTTOM;
        }
    }
    return W_SENSOR_OPENING_LEFT;
}

static bool sensor_parse_float(const char *text, float *out)
{
...
static int sensor_clamp_int(int v, int lo, int hi)
{
    if (v < lo) {
        return lo;
    }
...
static bool sensor_unit_is_percent(const char *unit)
{
    return unit != NULL && strcmp(unit, "%") == 0;
}

static int sensor_compute_percent(float value, const char *unit, int min, int max)
{
    if (sensor_unit_is_percent(unit)) {
        return sensor_clamp_int((int)(value + 0.5f), 0, 100);
    }
    if (max > min) {
        int p = (int)(((value - (float)min) / ((float)max - (float)min)) * 100.0f + 0.5f);
        return sensor_clamp_int(p, 0, 100);
    }
...
static void sensor_format_value(const char *state_text, const char *unit, char *buf, size_t len)
{
    if (unit != NULL && unit[0] != '\0') {
        if (sensor_unit_is_percent(unit)) {
            snprintf(buf, len, "%s%%", state_text);
        } else {
            snprintf(buf, len, "%s %s", state_text, unit);
        }
    } else {
        snprintf(buf, len, "%s", state_text);
    }
...
static void sensor_arc_angles(w_sensor_style_t style, w_sensor_opening_t opening, uint16_t *bg_start, uint16_t *bg_end)
{
    if (style == W_SENSOR_STYLE_ARC) {
        *bg_start = 135;
        *bg_end = 45;
        return;
    }
    switch (opening) {
        case W_SENSOR_OPENING_LEFT:
            *bg_start = 270;
            *bg_end = 90;
            break;
        case W_SENSOR_OPENING_RIGHT:
            *bg_start = 90;
            *bg_end = 270;
            break;
        case W_SENSOR_OPENING_TOP:
            *bg_start = 0;
            *bg_end = 180;
            break;
        case W_SENSOR_OPENING_BOTTOM:
        default:
            *bg_start = 180;
            *bg_end = 360;
            break;
...
static void sensor_apply_layout(w_sensor_ctx_t *ctx)
{
    ...
    if (ctx->style == W_SENSOR_STYLE_DEFAULT) {
    ...
    if (ctx->style == W_SENSOR_STYLE_PERCENT) {
        lv_obj_set_style_text_font(ctx->value_label, sensor_pick_value_font(ctx), LV_PART_MAIN);
        lv_obj_set_width(ctx->value_label, content_w);
        lv_obj_set_style_text_align(ctx->value_label, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN);

        lv_obj_align(ctx->value_label, LV_ALIGN_CENTER, 0, show_age ? -14 : -10);
        if (ctx->bar != NULL) {
            lv_obj_set_width(ctx->bar, content_w);
            lv_obj_align(ctx->bar, LV_ALIGN_BOTTOM_MID, 0, show_age ? -(lv_obj_get_height(ctx->bar) + 24) : -(lv_obj_get_height(ctx->bar) + 12));
        }
        if (show_age) {
            lv_obj_align(ctx->age_label, LV_ALIGN_BOTTOM_MID, 0, 0);
        }
        return;
    }
    ...
    lv_coord_t avail_h = content_h - title_h - (show_age ? 30 : 14);
    lv_coord_t arc_diam = (content_w < avail_h) ? content_w : avail_h;
    if (arc_diam < 40) {
        arc_diam = 40;
    }
    lv_obj_set_style_text_font(
        ctx->value_label,
        (arc_diam >= 150) ? SENSOR_VALUE_FONT_LARGE : ((arc_diam >= 90) ? SENSOR_VALUE_FONT_MEDIUM : SENSOR_VALUE_FONT_SMALL),
        LV_PART_MAIN);
    lv_obj_set_width(ctx->value_label, arc_diam);
    lv_obj_set_style_text_align(ctx->value_label, LV_TEXT_ALIGN_CENTER, LV_PART_MAIN);

    if (ctx->arc != NULL) {
        lv_obj_set_size(ctx->arc, arc_diam, arc_diam);
        lv_obj_align(ctx->arc, LV_ALIGN_CENTER, 0, show_age ? -8 : 0);
        lv_obj_align_to(ctx->value_label, ctx->arc, LV_ALIGN_CENTER, 0, 0);
    } else {
        lv_obj_align(ctx->value_label, LV_ALIGN_CENTER, 0, 0);
    }
...
esp_err_t w_sensor_create(const ui_widget_def_t *def, lv_obj_t *parent, ui_widget_instance_t *out_instance)
{
    ...
    w_sensor_style_t style = sensor_style_from_variant(def->style_variant);
    w_sensor_opening_t opening = sensor_opening_from_variant(def->arc_opening);
    lv_obj_t *bar = NULL;
    lv_obj_t *arc = NULL;

    if (style == W_SENSOR_STYLE_PERCENT) {
    ...
    } else if (style == W_SENSOR_STYLE_ARC || style == W_SENSOR_STYLE_ARC_SEMI) {
        ...
        uint16_t bg_start = 0;
        uint16_t bg_end = 0;
        sensor_arc_angles(style, opening, &bg_start, &bg_end);
...
void w_sensor_apply_state(ui_widget_instance_t *instance, const ha_state_t *state)
{
    ...
    sensor_format_value(state->state, unit, value_text, sizeof(value_text));
    ...
    if (ctx->bar != NULL || ctx->arc != NULL) {
        float fvalue = 0.0f;
        if (sensor_parse_float(state->state, &fvalue)) {
            int pct = sensor_compute_percent(fvalue, unit, ctx->min, ctx->max);
            if (ctx->bar != NULL) {
