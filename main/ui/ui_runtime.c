 ...
 * ("sensor.cpu_temp", used by entity_list/monitor_tile) or a labeled pair
 * ("GPU=sensor.gpu_temp", used by sensor_tile). Returns false when the token
 * has no usable entity part. */
static bool ui_runtime_token_entity(const char *start, const char *end, char *out, size_t out_len)
{
     ...
     * with an optional colour (":power:red" or ":power:#RRGGBB"). */
...
static void ui_runtime_apply_widget_all_states(ui_widget_instance_t *widget, bool mark_unavailable_if_missing)
{
    if (widget == NULL) {
        return;
    }
...
{
    ui_runtime_apply_widget_all_states(widget, mark_unavailable_if_missing);
}
...
{
    for (size_t i = 0; i < s_widget_count; i++) {
        ui_runtime_apply_widget_all_states(&s_widgets[i], true);
    }
    for (size_t i = 0; i < s_energy_page_count; i++) {
...
{
    for (size_t i = 0; i < s_widget_count; i++) {
        ui_runtime_apply_widget_all_states(&s_widgets[i], false);
    }
    for (size_t i = 0; i < s_energy_page_count; i++) {
        ui_energy_page_apply_all_states(&s_energy_pages[i]);
    }
...
static bool ui_runtime_widget_from_json(cJSON *widget_json, ui_widget_def_t *out)
{
    ...
    cJSON *sensor_min = cJSON_GetObjectItemCaseSensitive(widget_json, "sensor_min");
    cJSON *sensor_max = cJSON_GetObjectItemCaseSensitive(widget_json, "sensor_max");
    ...
    cJSON *card_bg_color = cJSON_GetObjectItemCaseSensitive(widget_json, "card_bg_color");
    cJSON *label_color = cJSON_GetObjectItemCaseSensitive(widget_json, "label_color");
    cJSON *bg_image = cJSON_GetObjectItemCaseSensitive(widget_json, "bg_image");
    ...
    const bool requires_entity = (strcmp(type->valuestring, "empty_tile") != 0 &&
                                  strcmp(type->valuestring, "entity_list") != 0 &&
    ...
    if (cJSON_IsNumber(sensor_min)) {
        out->sensor_min = sensor_min->valueint;
    }
    if (cJSON_IsNumber(sensor_max)) {
        out->sensor_max = sensor_max->valueint;
    }
    ...
    if (cJSON_IsString(card_bg_color) && card_bg_color->valuestring != NULL) {
        snprintf(out->card_bg_color, sizeof(out->card_bg_color), "%s", card_bg_color->valuestring);
    }
    if (cJSON_IsString(label_color) && label_color->valuestring != NULL) {
        snprintf(out->label_color, sizeof(out->label_color), "%s", label_color->valuestring);
    }
    if (cJSON_IsString(bg_image) && bg_image->valuestring != NULL) {
        snprintf(out->bg_image, sizeof(out->bg_image), "%s", bg_image->valuestring);
    }
    const cJSON *extra_entity_src = NULL;
    if (cJSON_IsString(extra_entity_ids) && extra_entity_ids->valuestring != NULL) {
        extra_entity_src = extra_entity_ids;
    } else if (cJSON_IsString(sub_entity_ids) && sub_entity_ids->valuestring != NULL) {
        extra_entity_src = sub_entity_ids;
    } else if (cJSON_IsString(list_entity_ids) && list_entity_ids->valuestring != NULL) {
        extra_entity_src = list_entity_ids;
    }
    if (extra_entity_src != NULL) {
        snprintf(out->extra_entity_ids, sizeof(out->extra_entity_ids), "%s", extra_entity_src->valuestring);
    }
    out->x = x->valueint;
    out->y = y->valueint;
    out->w = w->valueint;
