...
typedef struct {
    ...
    /* Entity-registry catalog import counters (see /api/ha/diagnostics). */
    bool catalog_done;
    uint32_t catalog_total;     /* Registry entries seen (before filtering). */
    uint32_t catalog_imported;  /* Inserted into the model (domain-allowed). */
    uint32_t catalog_disabled;  /* Imported entries flagged disabled_by. */
