...
typedef struct {
    ...
    /* Full entity-registry catalog import (config/entity_registry/list).
     * Unlike light discovery (which is domain-filtered and registry-mode is off
     * by default), this imports EVERY HA entity (including disabled ones) once
     * so the web editor's "Encja" picker and /api/entities expose the whole
     * device inventory (game-server binary_sensors, per-game RAM MB sensors,
     * PC Główny Glances sensors, Docker container sensors, ...) even when their
     * state is unavailable and would never produce a state_changed event. */
    bool entity_catalog_requested;
    bool entity_catalog_inflight;
    bool entity_catalog_done;
    uint32_t entity_catalog_req_id;
    uint32_t entity_catalog_total;
    uint32_t entity_catalog_imported;
    uint32_t entity_catalog_disabled;
    int64_t entity_catalog_next_retry_unix_ms;
...
static inline bool ha_client_heavy_in_flight_locked(void)
{
    return s_client.weather_ws_req_inflight ||
           ...
           s_client.entity_catalog_inflight ||
...
/* Full entity-registry catalog import: small post-auth cushion before the
 * first request, and the backoff between retries if HA rejects/loses it. */
static const int64_t HA_ENTITY_CATALOG_INITIAL_DELAY_MS = 2500;
static const int64_t HA_ENTITY_CATALOG_RETRY_DELAY_MS = 10000;
#if defined(CONFIG_APP_PANEL_VARIANT_S3_480)
...
#else
/* ESP32-P4 idles around ~110-115 KiB internal free with a ~36-40 KiB largest
 * contiguous block (DMA/internal reservations fragment the heap).  The
 * previous thresholds (180/80 KiB and 120/48 KiB) were never reachable on a
 * healthy idle panel, so entity discovery stayed stuck in
 * "internal_block_priority" forever and the entity picker returned nothing.
 * Lower them below the observed idle floor so the template-page discovery
 * (payload goes to PSRAM first) can actually run. */
static const size_t HA_LIGHT_DISCOVERY_MIN_INTERNAL_FREE_BYTES = (96U * 1024U);
static const size_t HA_LIGHT_DISCOVERY_MIN_INTERNAL_LARGEST_BYTES = (40U * 1024U);
static const size_t HA_LIGHT_DISCOVERY_TEMPLATE_MIN_INTERNAL_FREE_BYTES = (96U * 1024U);
static const size_t HA_LIGHT_DISCOVERY_TEMPLATE_MIN_INTERNAL_LARGEST_BYTES = (32U * 1024U);
#endif
#if defined(CONFIG_APP_PANEL_VARIANT_S3_480)
...
static const size_t HA_LIGHT_DISCOVERY_TEMPLATE_USER_MIN_INTERNAL_FREE_BYTES = (64U * 1024U);
static const size_t HA_LIGHT_DISCOVERY_TEMPLATE_USER_MIN_INTERNAL_LARGEST_BYTES = (24U * 1024U);
#endif
...
static esp_err_t ha_client_send_entity_catalog_request(void);
...
/* ESP32-P4 main RAM is PSRAM-backed with tens of MB free.  The full HA entity
 * registry catalog (config/entity_registry/list, including disabled entities)
 * is a single WS message that has been observed at ~1.05 MiB as the device
 * inventory grows, so the reassembly buffer must be much larger than the
 * default. */
static const size_t HA_WS_RX_ASSEMBLY_BUF_SIZE = 2097152U;
...
static void ha_client_heavy_watchdog_clear_locked(int64_t now_ms)
{
    ...
    if (s_client.entity_catalog_inflight) {
        s_client.entity_catalog_inflight = false;
        s_client.entity_catalog_req_id = 0;
        s_client.entity_catalog_requested = true;
        s_client.entity_catalog_next_retry_unix_ms = now_ms + HA_ENTITY_CATALOG_RETRY_DELAY_MS;
    }
...
/* Extract the entity part of one comma-list token. Supports both a plain
 * entity id ("sensor.cpu_temp") and a labeled pair ("GPU=sensor.gpu_temp").
 * Returns false when the token has no usable entity part. */
static bool ha_client_token_entity(const char *start, const char *end, char *out, size_t out_len)
{
    ...
    if (elen >= 4 && strncmp(end - 4, ":bar", 4) == 0) {
        ...
        /* ":power" may be followed by ":color" or "#RRGGBB". Locate it as a
         * token boundary so we don't mistake it for part of an entity id. */
        const char *p = start;
        while (p + 6 <= end) {
...
/* Collect every entity id found in a comma separated list value (used for
 * entity_ids / sub_entity_ids). Labeled "Label=entity" pairs contribute only
 * their entity part; plain entries contribute the whole token. */
...
static bool ha_client_entity_catalog_domain_allowed(const char *domain)
{
    /* Only import domains the panel can actually bind to a widget. The HA
     * registry is full of automation/script/update/device_tracker/person
     * entries that would otherwise exhaust APP_HA_MAX_ENTITIES and crowd out
     * the sensor/binary_sensor/switch/light entities the dashboard needs. */
    static const char *const ALLOWED_DOMAINS[] = {
        "sensor", "binary_sensor", "switch", "light", "climate",
        "weather", "todo", "media_player", "vacuum", "cover", "lock",
        "fan", "select", "number", "input_number",
    };
...
static void ha_client_entity_catalog_parse_locked(cJSON *result)
{
    /* config/entity_registry/list returns a bare array of registry entries
     * using full keys (entity_id, name, icon, hidden_by, disabled_by).
     * The legacy list_for_display shape {"entities":[...]} is still accepted
     * for backwards compatibility; its abbreviated keys are ei/en/ic/hb. */
    cJSON *entities = result;
    bool full_keys = true;
    if (cJSON_IsObject(result)) {
        cJSON *nested = cJSON_GetObjectItemCaseSensitive(result, "entities");
    ...
    cJSON_ArrayForEach(entry, entities)
    {
        ...
        s_client.entity_catalog_total++;

        ha_entity_info_t entity = {0};

        if (full_keys) {
            cJSON *hidden_by = cJSON_GetObjectItemCaseSensitive(entry, "hidden_by");
            if (cJSON_IsString(hidden_by) && hidden_by->valuestring != NULL &&
                hidden_by->valuestring[0] != '\0') {
                continue;
            }
            cJSON *disabled_by = cJSON_GetObjectItemCaseSensitive(entry, "disabled_by");
            entity.disabled = cJSON_IsString(disabled_by) && disabled_by->valuestring != NULL &&
                              disabled_by->valuestring[0] != '\0';
            if (!ha_client_json_string_value(entry, "entity_id", entity.id, sizeof(entity.id)) ||
                entity.id[0] == '\0') {
                continue;
            }
            if (!ha_client_json_string_value(entry, "name", entity.name, sizeof(entity.name))) {
                safe_copy_cstr(entity.name, sizeof(entity.name), entity.id);
            }
            (void)ha_client_json_string_value(entry, "icon", entity.icon, sizeof(entity.icon));
        } else {
            cJSON *hidden_by = cJSON_GetObjectItemCaseSensitive(entry, "hb");
            if (cJSON_IsTrue(hidden_by)) {
                continue;
            }
            if (!ha_client_json_string_value(entry, "ei", entity.id, sizeof(entity.id)) ||
                entity.id[0] == '\0') {
                continue;
            }
            if (!ha_client_json_string_value(entry, "en", entity.name, sizeof(entity.name))) {
                safe_copy_cstr(entity.name, sizeof(entity.name), entity.id);
            }
            (void)ha_client_json_string_value(entry, "ic", entity.icon, sizeof(entity.icon));
        }

        /* Derive domain from entity_id ("sensor.foo" -> "sensor"). */
        const char *dot = strchr(entity.id, '.');
        if (dot != NULL) {
            size_t domain_len = (size_t)(dot - entity.id);
            if (domain_len >= sizeof(entity.domain)) {
                domain_len = sizeof(entity.domain) - 1U;
            }
            memcpy(entity.domain, entity.id, domain_len);
            entity.domain[domain_len] = '\0';
        } else {
            safe_copy_cstr(entity.domain, sizeof(entity.domain), "unknown");
        }

        if (!ha_client_entity_catalog_domain_allowed(entity.domain)) {
            continue;
        }

        if (ha_model_ensure_entity(&entity) == ESP_OK) {
            s_client.entity_catalog_imported++;
            if (entity.disabled) {
                s_client.entity_catalog_disabled++;
            }
        }
    }
}
...
static esp_err_t ha_client_send_entity_catalog_request(void)
{
    ...
    uint32_t req_id = ha_client_next_message_id();
    cJSON_AddNumberToObject(root, "id", (double)req_id);
    cJSON_AddStringToObject(root, "type", "config/entity_registry/list");

    xSemaphoreTake(s_client.mutex, portMAX_DELAY);
    s_client.entity_catalog_req_id = req_id;
    s_client.entity_catalog_inflight = true;
    s_client.entity_catalog_requested = false;
    s_client.entity_catalog_next_retry_unix_ms = 0;
    xSemaphoreGive(s_client.mutex);

    ESP_LOGI(TAG_HA_CLIENT, "Requesting HA entity registry catalog (list)");
    esp_err_t err = ha_client_send_json(root);
    cJSON_Delete(root);
    if (err != ESP_OK) {
        xSemaphoreTake(s_client.mutex, portMAX_DELAY);
        if (s_client.entity_catalog_req_id == req_id) {
            s_client.entity_catalog_inflight = false;
            s_client.entity_catalog_requested = true;
            s_client.entity_catalog_req_id = 0;
            s_client.entity_catalog_next_retry_unix_ms = ha_client_now_ms() + HA_ENTITY_CATALOG_RETRY_DELAY_MS;
        }
        xSemaphoreGive(s_client.mutex);
        ESP_LOGW(TAG_HA_CLIENT, "Failed to request HA entity catalog: %s", esp_err_to_name(err));
    }
    return err;
}
...
static bool ha_client_entity_catalog_handle_result(uint32_t msg_id, cJSON *root, cJSON *success_item)
{
    bool is_entity_catalog = false;

    xSemaphoreTake(s_client.mutex, portMAX_DELAY);
    if (s_client.entity_catalog_inflight && s_client.entity_catalog_req_id == msg_id) {
        is_entity_catalog = true;
        ha_client_ws_send_gate_mark_heavy_done_locked(ha_client_now_ms());
        s_client.entity_catalog_inflight = false;
        s_client.entity_catalog_req_id = 0;
    }
    ...
    xSemaphoreTake(s_client.mutex, portMAX_DELAY);
    if (success && (cJSON_IsArray(result) || cJSON_IsObject(result))) {
        s_client.entity_catalog_imported = 0;
        s_client.entity_catalog_disabled = 0;
        s_client.entity_catalog_total = 0;
        ha_client_entity_catalog_parse_locked(result);
        s_client.entity_catalog_done = true;
        uint32_t total = s_client.entity_catalog_total;
        uint32_t imported = s_client.entity_catalog_imported;
        uint32_t disabled = s_client.entity_catalog_disabled;
        xSemaphoreGive(s_client.mutex);
        ESP_LOGI(TAG_HA_CLIENT,
            "HA entity catalog imported %" PRIu32 " of %" PRIu32 " registry entities (%" PRIu32 " disabled)",
            imported, total, disabled);
    } else {
        s_client.entity_catalog_requested = true;
        s_client.entity_catalog_next_retry_unix_ms =
            ha_client_now_ms() + HA_ENTITY_CATALOG_RETRY_DELAY_MS;
        xSemaphoreGive(s_client.mutex);
        if (success) {
            ESP_LOGW(TAG_HA_CLIENT, "HA entity catalog returned an unexpected result, retrying");
        } else {
            ESP_LOGW(TAG_HA_CLIENT, "HA entity catalog request failed, retrying");
        }
    }
...
static void ha_client_handle_result_message(cJSON *root)
{
    ...
    if (ha_client_entity_catalog_handle_result(msg_id, root, success_item)) {
        return;
    }
...
static void ha_client_handle_text_message(const char *data, int len)
{
    ...
    if (strcmp(type->valuestring, "auth_required") == 0) {
        ...
        s_client.pending_subscribe = APP_HA_SUBSCRIBE_STATE_CHANGED;
        /* Re-arm the full entity-registry catalog import for this session.
         * `ensure_entity` is insert-if-absent, so a re-import after a model
         * reset or a HA restart is harmless and self-healing. */
        s_client.entity_catalog_requested = true;
        s_client.entity_catalog_inflight = false;
        s_client.entity_catalog_done = false;
        s_client.entity_catalog_req_id = 0;
        s_client.entity_catalog_total = 0;
        s_client.entity_catalog_imported = 0;
        s_client.entity_catalog_disabled = 0;
        s_client.entity_catalog_next_retry_unix_ms = now_ms + HA_ENTITY_CATALOG_INITIAL_DELAY_MS;
        rest_enabled = s_client.rest_enabled;
...
static void ha_client_ws_event_cb(const ha_ws_event_t *event, void *user_ctx)
{
    ...
    case HA_WS_EVENT_DISCONNECTED:
        ...
        s_client.energy_prefs_req_id = 0;
        s_client.energy_stats_req_id = 0;
        /* Entity catalog is re-armed on the next auth_ok. */
        s_client.entity_catalog_inflight = false;
        s_client.entity_catalog_requested = false;
        s_client.entity_catalog_req_id = 0;
        s_client.entity_catalog_next_retry_unix_ms = 0;
...
static void ha_client_task(void *arg)
{
    ...
    while (true) {
        ...
        bool entity_catalog_requested = false;
        bool entity_catalog_inflight = false;
        int64_t entity_catalog_next_retry_unix_ms = 0;
        ...
        entity_catalog_requested = s_client.entity_catalog_requested;
        entity_catalog_inflight = s_client.entity_catalog_inflight;
        entity_catalog_next_retry_unix_ms = s_client.entity_catalog_next_retry_unix_ms;
        ...
        /* Full entity-registry catalog import.  Runs once the initial layout
         * sync has settled so the heavy list_for_display response cannot race
         * the get_states / subscribe roundtrips.  Imported entities populate
         * /api/entities and the editor "Encja" picker without needing a live
         * state (unavailable sensors never emit state_changed). */
        if (connected && authenticated && initial_layout_sync_done &&
            entity_catalog_requested && !entity_catalog_inflight &&
            now_ms >= entity_catalog_next_retry_unix_ms) {
            bool catalog_stable = !pending_subscribe &&
                !pending_get_states &&
                !ping_inflight &&
                !should_send_ping &&
                !should_run_priority_sync_step &&
            ...
            if (catalog_stable) {
                ...
                gate_ok = ha_client_ws_send_gate_ok_locked(HA_WS_SEND_HEAVY, now_ms);
                xSemaphoreGive(s_client.mutex);
                if (gate_ok && ha_client_send_entity_catalog_request() != ESP_OK) {
                    xSemaphoreTake(s_client.mutex, portMAX_DELAY);
                    s_client.entity_catalog_requested = true;
                    s_client.entity_catalog_inflight = false;
                    s_client.entity_catalog_next_retry_unix_ms =
                        now_ms + HA_ENTITY_CATALOG_RETRY_DELAY_MS;
                    xSemaphoreGive(s_client.mutex);
                }
            }
        }
...
void ha_client_get_diagnostics(ha_client_diagnostics_t *out)
{
    ...
    out->catalog_done = s_client.entity_catalog_done;
    out->catalog_total = s_client.entity_catalog_total;
    out->catalog_imported = s_client.entity_catalog_imported;
    out->catalog_disabled = s_client.entity_catalog_disabled;
