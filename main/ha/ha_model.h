...
/* Insert an entity only if it is not already present.  Used by the full
 * entity-registry catalog import so that a later, richer upsert (state import
 * with unit/device_class) is never overwritten by a name-only registry row. */
esp_err_t ha_model_ensure_entity(const ha_entity_info_t *entity);
