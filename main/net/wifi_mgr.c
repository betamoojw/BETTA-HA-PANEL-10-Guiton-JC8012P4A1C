...
static esp_err_t wifi_mgr_apply_static_ip(esp_netif_t *netif, const wifi_mgr_config_t *cfg)
{
    if (cfg == NULL || netif == NULL || !cfg->static_ip_enabled) {
        return ESP_OK;
    }

    esp_netif_ip_info_t ip_info = {0};
    if (esp_netif_str_to_ip4(cfg->static_ip, &ip_info.ip) != ESP_OK ||
        esp_netif_str_to_ip4(cfg->static_netmask, &ip_info.netmask) != ESP_OK ||
        esp_netif_str_to_ip4(cfg->static_gateway, &ip_info.gw) != ESP_OK) {
        ESP_LOGW(TAG_WIFI, "Invalid static IPv4 configuration, keeping DHCP");
        return ESP_ERR_INVALID_ARG;
    }

    esp_err_t err = esp_netif_dhcpc_stop(netif);
    if (err != ESP_OK && err != ESP_ERR_ESP_NETIF_DHCP_ALREADY_STOPPED) {
        ESP_LOGW(TAG_WIFI, "dhcpc_stop failed: %s", esp_err_to_name(err));
    }

    err = esp_netif_set_ip_info(netif, &ip_info);
    if (err != ESP_OK) {
        ESP_LOGW(TAG_WIFI, "esp_netif_set_ip_info failed: %s; restoring DHCP", esp_err_to_name(err));
        (void)esp_netif_dhcpc_start(netif);
        return err;
    }

    if (cfg->static_dns != NULL && cfg->static_dns[0] != '\0') {
        esp_netif_dns_info_t dns = {0};
        if (esp_netif_str_to_ip4(cfg->static_dns, &dns.ip.u_addr.ip4) == ESP_OK) {
            dns.ip.type = ESP_IPADDR_TYPE_V4;
            (void)esp_netif_set_dns_info(netif, ESP_NETIF_DNS_MAIN, &dns);
        }
    }

    ESP_LOGI(TAG_WIFI, "Static IPv4 applied: %s/%s gw %s",
...
esp_err_t wifi_mgr_init(const wifi_mgr_config_t *cfg)
{
    ...
    (void)wifi_mgr_apply_static_ip(s_wifi_sta_netif, cfg);
