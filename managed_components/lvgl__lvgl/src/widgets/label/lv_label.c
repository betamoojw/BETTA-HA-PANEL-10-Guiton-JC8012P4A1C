...
void lv_label_get_letter_pos(const lv_obj_t * obj, uint32_t char_id, lv_point_t * pos)
{
    ...
    calculate_x_coordinate(&x, align, bidi_txt, length, font, &txt_coords, &attributes);
    pos->x = x;
    pos->y = y;
...
static void lv_label_constructor(const lv_obj_class_t * class_p, lv_obj_t * obj)
{
    ...
    /* Panel fix: LVGL v9 marks every object scrollable by default. In this
     * fixed-layout dashboard labels never scroll, and a stray scrollable label
     * in the pressed object's ancestor chain becomes the indev scroll_obj,
     * which makes sliders ignore drag gestures (lv_slider.c update_knob_pos). */
