...
static uint8_t * sdio_rx_get_buffer(uint32_t len)
{
	...
	if (len > double_buf.buffer[index].buf_size) {
		/* Allocate the new, larger buffer before freeing the old one so an
		 * allocation failure leaves the previous buffer usable. */
		uint8_t *new_buf = (uint8_t *)g_h.funcs->_h_malloc_align(len, HOSTED_MEM_ALIGNMENT_64);
		if (!new_buf) {
			ESP_LOGE(TAG, "sdio rx buffer alloc failed (%u bytes), drop rx", (unsigned)len);
			return NULL;
		}
		if (*buf) {
...
static esp_err_t sdio_push_data_to_queue(uint8_t * buf, uint32_t buf_len)
{
	...
	do {
		...
		if (!pkt_rxbuff) {
			ESP_LOGE(TAG, "sdio rx packet buffer alloc failed, drop rx stream");
			return ESP_FAIL;
		}
